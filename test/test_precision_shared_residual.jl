...
function _shared_residual_dense_nll(Y, phy, beta, loading, residual_sd, species_id)
    ...
    residual = vec(Y) - GLLVModels._trait_mean_design(d, m) * beta
...
@testset "precision multivariate shared residual" begin
    ...
    d, m = size(fixture.Y)
    log_sd = log(0.55)
    shared_theta = vcat(fixture.beta, GLLVModels.pack_lambda(fixture.loading), log_sd)
    trait_theta = vcat(fixture.beta, GLLVModels.pack_lambda(fixture.loading),
        fill(log_sd, d))

    shared = GLLVModels._precision_multivariate_unpack(shared_theta, d, 1,
        :barelowrank, d; residual_mode=:shared)
    trait = GLLVModels._precision_multivariate_unpack(trait_theta, d, 1,
        :barelowrank, d)
    @test length(shared_theta) == GLLVModels._pmv_layout(d, 1, :barelowrank, d;
        residual_mode=:shared).total == 7
    @test length(trait_theta) == GLLVModels._pmv_layout(d, 1, :barelowrank, d).total == 9
    ...
    sparse_shared = GLLVModels._precision_multivariate_nll(fixture.Y, fixture.phy,
        shared_theta; rank=1, mode=:barelowrank, residual_mode=:shared,
        species_id=fixture.species_id)
    sparse_trait = GLLVModels._precision_multivariate_nll(fixture.Y, fixture.phy,
    ...
    shared_fit = GLLVModels.fit_precision_multivariate(fixture.Y, fixture.phy;
        rank=1, mode=:barelowrank, residual_mode=:shared,
        species_id=fixture.species_id, start=shared_theta, iterations=0)
    trait_fit = GLLVModels.fit_precision_multivariate(fixture.Y, fixture.phy;
    ...
    shared_targets = GLLVModels._pmv_targets(shared_fit)
    ...
    @test GLLVModels._destination_b_fixed_objective(shared_fit)(shared_fit.parameters) ≈
        sparse_shared atol=1e-10
    public_fit = GLLVModels.fit_gllvm(fixture.Y; family=GLLVModels.Normal(), phylo=fixture.phy,
        phylo_rank=1, phylo_mode=:barelowrank, residual_mode=:shared,
        species_id=fixture.species_id, start=shared_theta, iterations=0)
    @test public_fit isa GLLVModels.PrecisionMultivariateFit
    @test public_fit.residual_mode === :shared
    @test public_fit.parameters == shared_theta
    @test_throws ArgumentError GLLVModels._precision_multivariate_unpack(shared_theta, d, 1,
