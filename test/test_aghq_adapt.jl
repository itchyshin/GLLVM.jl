...
function _liu_pierce_site_golden(family, y, n, Λ, β, link; k::Integer,
    ...
    d = size(Λ, 2)
    p = size(Λ, 1)
    grid = GLLVModels.aghq_grid(d, k)
    z = GLLVModels._laplace_mode(family, y, n, Λ, β, link)
    Λz = Λ * z
    η = GLLVModels._clamp_eta.(β .+ Λz)
    μ = GLLVModels._clamp_mu.(Ref(family), GLLVModels.linkinv.(Ref(link), η))
    me = GLLVModels.mu_eta.(Ref(link), η)
    W = GLLVModels._glm_weight.(Ref(family), μ, n, me)
    Amat = Λ' * (W .* Λ)
    ...
    @inbounds for j in 1:ngrid
        ...
        Λzj = Λ * zj
        ηj = GLLVModels._clamp_eta.(β .+ Λzj)
        μj = GLLVModels._clamp_mu.(Ref(family), GLLVModels.linkinv.(Ref(link), ηj))
        ℓj = 0.0
        for t in 1:p
            ℓj += GLLVModels._glm_logpdf(family, μj[t], n[t], y[t])
        end
...
@testset "AGHQ Stage-1b Liu–Pierce adaptation" begin
    ...
    @testset "k=1 marginal still matches poisson Laplace" begin
        ...
        aghq = GLLVModels.aghq_stage1a_marginal_loglik(Poisson(), Y, N, Λ, β,
                                                  GLLVModels.LogLink(); k = 1)
