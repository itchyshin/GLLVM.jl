...
@testset "AGHQ Stage-1a live-pin grid" begin
    ...
    @testset "probabilists' rule is not VA _gauss_hermite" begin
        ...
        x_phys, w_phys = GLLVModels._gauss_hermite(3)
        x_prob, w_prob = GLLVModels._aghq_gh_normal(3)
    ...
    @testset "k=1 marginal matches poisson Laplace" begin
        ...
        aghq = GLLVModels.aghq_stage1a_marginal_loglik(Poisson(), Y, N, Λ, β, GLLVModels.LogLink(); k = 1)
        @test isfinite(lap) && isfinite(aghq)
        @test aghq ≈ lap atol = 1e-12
    end
    ...
    @testset "k>1 site evaluator exercises Liu–Pierce nodes" begin
        ...
        link = GLLVModels.LogLink()
        fam = Poisson()
        ll1 = GLLVModels.aghq_stage1a_loglik_site(fam, y, n, Λ, β, link; k = 1)
        ll3 = GLLVModels.aghq_stage1a_loglik_site(fam, y, n, Λ, β, link; k = 3)
    ...
    @testset "k=1 site loglik matches dense Laplace under each family's DEFAULT hessian" begin
        ...
        cases = (
            (fam = Gamma(3.0, 1.0), link = GLLVModels.LogLink(),
             y = 0.4 .+ rand(p), n = ones(Int, p)),
            (fam = NegativeBinomial(4.0, 0.5), link = GLLVModels.LogLink(),
             y = Float64.(rand(0:8, p)), n = ones(Int, p)),
            (fam = Beta(8.0, 1.0), link = GLLVModels.LogitLink(),
             y = clamp.(rand(p), 0.02, 0.98), n = ones(Int, p)),
            (fam = GLLVModels.NB1(1.5), link = GLLVModels.LogLink(),
             y = Float64.(rand(0:8, p)), n = ones(Int, p)),
            (fam = GLLVModels.StudentTFamily(4.0, 1.0), link = GLLVModels.IdentityLink(),
             y = randn(p), n = ones(Int, p)),
            (fam = Exponential(1.0), link = GLLVModels.LogLink(),
             y = 0.3 .+ rand(p), n = ones(Int, p)),
            (fam = GLLVModels.TruncatedNegBin2(), link = GLLVModels.LogLink(),
             y = Float64.(rand(1:8, p)), n = ones(Int, p)),
            (fam = GLLVModels.TweedieED(1.2, 1.5), link = GLLVModels.LogLink(),
             y = [rand() < 0.3 ? 0.0 : 0.2 + 2 * rand() for _ in 1:p], n = ones(Int, p)),
            (fam = Binomial(), link = GLLVModels.ProbitLink(),
             y = Float64.(rand(0:6, p)), n = fill(6, p)),
        )

        for c in cases
            @test GLLVModels._default_hessian(c.fam, c.link) === :observed
            lap  = GLLVModels.laplace_loglik_site(c.fam, c.y, c.n, Λ, β, c.link)
            aghq = GLLVModels.aghq_stage1a_loglik_site(c.fam, c.y, c.n, Λ, β, c.link; k = 1)
            @test isfinite(lap) && isfinite(aghq)
    ...
    @testset "fail-loud: extra random structure (loadings-only z_B)" begin
        ...
        link = GLLVModels.LogLink()
        fam = Poisson()
        @test_throws ArgumentError GLLVModels.aghq_stage1a_loglik_site(
        ...
        @test_throws ArgumentError GLLVModels.aghq_grid(0, 3)
        @test_throws ArgumentError GLLVModels.aghq_grid(1, 0)
    end
end
