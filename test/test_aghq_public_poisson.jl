...
@testset "PU public Poisson AGHQ" begin
    @test hasfield(GLLVModels.PoissonFit,:integration)
    if hasfield(GLLVModels.PoissonFit,:integration)
        ...
        old=GLLVModels.PoissonFit(base.β,base.Λ,base.link,base.loglik,base.converged,base.iterations)
        ...
        @test GLLVModels._aghq_request(true)===:auto
        @test GLLVModels._aghq_request(nothing)===:off
        @test GLLVModels._aghq_request(2)==2
        ...
        replay=GLLVModels._poisson_aghq_refit_kwargs(fit)
        ...
        @test_throws ArgumentError GLLVModels._family_ci(fit,Y.+1;objective=:fit)
        ...
        @test_throws ArgumentError GLLVModels._family_ci(fit,Y;mask=trues(size(Y)))
...
@testset "PU04 Wald positive definiteness" begin
    ...
    ad=GLLVModels._FamilyCI(zeros(3),x->dot(x,H*x)/2,["a","b","c"],fill(:linear,3),r->zeros(1,1),y->zeros(3))
    result=GLLVModels._family_wald(ad,[1,2,3],.95)
    @test !result.pd_hessian
    @test all(isnan,result.se)
    good=GLLVModels._FamilyCI(zeros(3),x->sum(abs2,x)/2,["a","b","c"],fill(:linear,3),r->zeros(1,1),y->zeros(3))
    @test GLLVModels._family_wald(good,[1,2,3],.95).se ≈ ones(3)
end
