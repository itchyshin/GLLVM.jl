using GLLVModels, Test, Distributions, Random, LinearAlgebra

# exact independent-binomial loglik (no latent variation)
_indep_binom_loglik(y, n, β, link) = sum(
    logpdf(Binomial(Int(n[t]), clamp(GLLVModels.linkinv(link, β[t]), 1e-12, 1 - 1e-12)), Int(y[t]))
...
function _quad_marginal_k1(y, n, Λ, β, link; lo = -12.0, hi = 12.0, m = 6001)
    ...
    f(z) = exp(sum(
        logpdf(Binomial(Int(n[t]), clamp(GLLVModels.linkinv(link, β[t] + Λ[t, 1] * z), 1e-12, 1 - 1e-12)), Int(y[t]))
...
@testset "Binomial Laplace marginal" begin
    ...
    @testset "Λ=0 ⇒ Laplace is exact (independent binomial)" begin
        ...
        for link in (LogitLink(), ProbitLink(), CLogLogLink())
            lap = GLLVModels.laplace_loglik_site(y, n, Λ0, β, link)
    ...
    @testset "K=1: Laplace ≈ exact marginal (quadrature)" begin
        ...
        for link in (LogitLink(), ProbitLink())
            lap = GLLVModels.laplace_loglik_site(y, n, Λ, β, link)
    ...
    @testset "binomial trials n>1 + multi-site additivity" begin
        ...
        tot = GLLVModels.binomial_marginal_loglik_laplace(Y, N, Λ, β, LogitLink())
        s1 = GLLVModels.laplace_loglik_site(view(Y, :, 1), view(N, :, 1), Λ, β, LogitLink())
        s2 = GLLVModels.laplace_loglik_site(view(Y, :, 2), view(N, :, 2), Λ, β, LogitLink())
    ...
    @testset "separated data does not blow up" begin
        Λ = reshape(0.5 .* randn(p), p, 1)
        @test isfinite(GLLVModels.laplace_loglik_site(fill(1, p), fill(1, p), Λ, fill(10.0, p), LogitLink()))
        @test isfinite(GLLVModels.laplace_loglik_site(fill(0, p), fill(1, p), Λ, fill(-10.0, p), LogitLink()))
    end
end
