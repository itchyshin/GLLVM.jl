using GLLVModels, Test, TOML, ForwardDiff
...
function retained_student_reference(x, y)
    ...
    half = (nu+1)/2
    GLLVModels.loggamma(half)-GLLVModels.loggamma(nu/2)-log(nu*big(π))/2-ls-
...
@testset "Student-t retained fitted parameter scalar precision" begin
 setprecision(BigFloat,768) do
  for point in points,t in 1:5,z in (-3.0,-0.5,0.0,0.5,3.0)
   ...
   f(v)=GLLVModels._glm_logpdf(GLLVModels.StudentTFamily(1+exp(v[3]),exp(v[2])),v[1],1,y)
   ...
   fam=GLLVModels.StudentTFamily(1+exp(x[3]),exp(x[2]))
   @test isapprox(sigma*GLLVModels._glm_score(fam,0.0,1,1.0,y),sigma*gradient[1];atol=1e-10,rtol=1e-9)
   @test isapprox(sigma^2*GLLVModels._glm_obs_weight(fam,0.0,1,1.0,y,GLLVModels.IdentityLink(),0.0),-sigma^2*H[1,1];atol=1e-8,rtol=1e-8)
  end
 end
end
