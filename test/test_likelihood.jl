...
@testset "likelihood" begin
    @testset "matches direct MvNormal log-pdf" begin
        ...
        # ours:
        ll_ours   = GLLVModels.gaussian_marginal_loglik(y, Λ, σ)
    ...
    @testset "scales correctly with σ → large" begin
        ...
        y = randn(p, n)
        ll_big = GLLVModels.gaussian_marginal_loglik(y, Λ, 100.0)
    ...
    @testset "AD-friendly" begin
        ...
        σ = 0.5
        y = randn(p_local, n)
        params = [log(σ); GLLVModels.pack_lambda(Λ)]
        g = ForwardDiff.gradient(prm -> GLLVModels.gaussian_nll_packed(prm, y, p_local, K_local), params)
        @test all(isfinite, g)
        @test length(g) == 1 + GLLVModels.rr_theta_len(p_local, K_local)
    end
end
