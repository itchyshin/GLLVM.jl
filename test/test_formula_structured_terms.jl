...
# call the internal `GLLVModels._...` functions directly on raw quoted `Expr`s.
...
@testset "Structured-term recognizer: Step 0 scaffolding" begin
    ...
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(indep(1 + x | g)))
    # Red: non-literal flag aborts (R .read_common_flag, brms-sugar.R:2464-2483)
    flagvar = true
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(indep(0 + trait | g, common = flagvar)))
    # Green: both valid bar-LHS shapes parse and carry the group symbol.
    spec1 = GLLVModels._recognize_source_term(:(indep(0 + trait | g)))
    @test spec1.kind === :indep && spec1.group === :g && spec1.common === false
    spec2 = GLLVModels._recognize_source_term(:(dep(1 | grp)))
    @test spec2.kind === :dep && spec2.group === :grp
    # Non-bar first argument aborts.
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(indep(g)))
    # Unrecognized call name aborts.
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(not_a_term(0 + trait | g)))
end
...
@testset "Structured-term recognizer: Step 1 indep()" begin
    ...
    spec = GLLVModels._recognize_source_term(:(indep(0 + trait | g)))
    @test spec.common === false
    source = GLLVModels._source_term_covariance(spec, data)
    @test source.mode === :indep && source.common == false
    @test source.covariance == Matrix{Float64}(I, 6, 6)

    formula_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(indep(0 + trait | g))]; opts...)
    ...
    spec_common = GLLVModels._recognize_source_term(:(indep(0 + trait | g, common = true)))
    @test spec_common.common === true
    source_common = GLLVModels._source_term_covariance(spec_common, data)
    @test GLLVModels._source_nparams(source_common, p) == 1
    fit_common = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(indep(0 + trait | g, common = true))]; opts...)
    direct_common = fit_gaussian_sources(Y; sources = [source_common], opts...)
    @test fit_common.loglik ≈ direct_common.loglik atol = 1e-8

    @test_throws ArgumentError GLLVModels._recognize_source_term(:(indep(0 + trait | g, bogus = true)))
end
...
@testset "Structured-term recognizer: Step 2 scalar() alias + warn" begin
    ...
    scalar_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(scalar(0 + trait | g))]; opts...)
    indep_common_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(indep(0 + trait | g, common = true))]; opts...)
    @test scalar_fit.loglik ≈ indep_common_fit.loglik atol = 1e-8
    @test scalar_fit.beta ≈ indep_common_fit.beta atol = 1e-8

    scalar_spec = GLLVModels._recognize_source_term(:(scalar(0 + trait | g)))
    @test scalar_spec.kind === :scalar && scalar_spec.common === true

    GLLVModels._SCALAR_DEPRECATION_WARNED[] = false
    @test_logs (:warn, r"deprecated") GLLVModels._recognize_source_term(:(scalar(1 | g)))
    # One-shot: a second call in the same session does not re-warn.
    @test_logs GLLVModels._recognize_source_term(:(scalar(1 | g)))
    @test GLLVModels._SCALAR_DEPRECATION_WARNED[] === true
end
...
@testset "Structured-term recognizer: Step 3 dep()" begin
    ...
    spec = GLLVModels._recognize_source_term(:(dep(0 + trait | g)))
    @test spec.kind === :dep
    source = GLLVModels._source_term_covariance(spec, data)
    @test source.mode === :dep
    @test GLLVModels._source_nparams(source, p) == GLLVModels.rr_theta_len(p, p)

    formula_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(dep(0 + trait | g))]; opts...)
    direct = fit_gaussian_sources(Y; sources = [source], opts...)
    @test formula_fit.loglik ≈ direct.loglik atol = 1e-8
    @test formula_fit.beta ≈ direct.beta atol = 1e-7

    # rank= is not a dep() spelling — reject any keyword.
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(dep(0 + trait | g, rank = 2)))
end
...
@testset "Structured-term recognizer: Step 4 mutual-exclusion gates" begin
    ...
    specs(exprs) = GLLVModels.SourceTermSpec[GLLVModels._recognize_source_term(e) for e in exprs]

    @test_throws ArgumentError GLLVModels._check_source_term_exclusions(specs([dep_g, indep_g]))
    @test_throws ArgumentError GLLVModels._check_source_term_exclusions(specs([dep_g, scalar_g]))
    @test_throws ArgumentError GLLVModels._check_source_term_exclusions(specs([dep_g, klatent_g]))
    ...
    @test GLLVModels._check_source_term_exclusions(specs([indep_g, klatent_g])) === nothing
    @test_throws ArgumentError GLLVModels._check_source_term_exclusions(specs([dep_g, klatent_unique_g]))
    @test_throws ArgumentError GLLVModels._check_source_term_exclusions(specs([dep_g, kunique_g]))
    # Non-conflicting: different groupings never gate each other.
    @test GLLVModels._check_source_term_exclusions(specs([dep_g, :(indep(0 + trait | h))])) === nothing
    ...
    @test GLLVModels._check_source_term_exclusions(specs([indep_g, indep_g])) === nothing
end
...
@testset "Structured-term recognizer: Step 5 kernel_indep/scalar/dep()" begin
    ...
    @test_throws ArgumentError GLLVModels._recognize_source_term(:(kernel_indep(g)))
    ...
    for (kind, expr) in ((:kernel_indep, :(kernel_indep(g, K = K, name = "known"))),
        ...
        spec = GLLVModels._recognize_source_term(expr)
        @test spec.kind === kind && spec.name === :known
        source = GLLVModels._source_term_covariance(spec, data; kernel_env = kernel_env)
        @test source.covariance ≈ K
        fitted = GLLVModels._fit_gaussian_structured_sources(Y, data, [expr]; kernel_env = kernel_env, opts...)
        direct = fit_gaussian_sources(Y; sources = [source], opts...)
        @test fitted.loglik ≈ direct.loglik atol = 1e-8
    end

    common_spec = GLLVModels._recognize_source_term(:(kernel_indep(g, K = K, common = true)))
...
@testset "Structured-term recognizer: Step 6 kernel_latent()" begin
    ...
    spec = GLLVModels._recognize_source_term(:(kernel_latent(g, K = K, d = 2, name = "k1")))
    @test spec.kind === :kernel_latent && spec.d == 2 && spec.name === :k1 && spec.unique === false
    source = GLLVModels._source_term_covariance(spec, data; kernel_env = kernel_env)
    @test source.mode === :latent && source.rank == 2 && !source.unique

    formula_fit = GLLVModels._fit_gaussian_structured_sources(Y, data,
    ...
    unique_spec = GLLVModels._recognize_source_term(:(kernel_latent(g, K = K, d = 1, name = "k1", unique = true)))
    @test unique_spec.unique === true
    unique_source = GLLVModels._source_term_covariance(unique_spec, data; kernel_env = kernel_env)
    @test unique_source.unique
    @test GLLVModels._source_nparams(unique_source, p) ==
        GLLVModels.rr_theta_len(p, 1) + p

    unique_fit = GLLVModels._fit_gaussian_structured_sources(Y, data,
    ...
    flagvar = true
    @test_throws ArgumentError GLLVModels._recognize_source_term(
    ...
    ku_spec = GLLVModels._recognize_source_term(:(kernel_unique(g, K = K, name = "k1")))
    @test ku_spec.kind === :kernel_unique
    @test_throws ArgumentError GLLVModels._source_term_covariance(ku_spec, data; kernel_env = kernel_env)
    ...
    bad_spec = GLLVModels._recognize_source_term(:(kernel_latent(g, K = Kbad, d = 1)))
    @test_throws ArgumentError GLLVModels._source_term_covariance(bad_spec, data; kernel_env = (Kbad = Kbad,))
end
...
@testset "fit_gaussian_structured: public structure= wrapper (round2-3 #6)" begin
    ...
    internal_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, [:(indep(0 + trait | g))]; opts...)
    ...
    internal_kernel_fit = GLLVModels._fit_gaussian_structured_sources(Y, data, structure;
        kernel_env = kernel_env, opts...)
    ...
    # silent no-op.
    @test_throws ArgumentError fit_gaussian_structured(Y, data;
        structure = [:(indep(0 + trait | g))], family = GLLVModels.Distributions.Poisson(), opts...)
