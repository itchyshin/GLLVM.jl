using Test, GLLVModels, LinearAlgebra, Random, SHA, SparseArrays, StableRNGs

@testset "private grouped non-Gaussian Laplace fitter" begin
    @test isdefined(GLLVModels, :GroupedNonGaussianFit)
    @test isdefined(GLLVModels, :fit_grouped_nongaussian)
    @test isdefined(GLLVModels, :grouped_nongaussian_intervals)
    if isdefined(GLLVModels, :GroupedNonGaussianFit) &&
            isdefined(GLLVModels, :fit_grouped_nongaussian) &&
            isdefined(GLLVModels, :grouped_nongaussian_intervals)
        term = GLLVModels.GroupingTerm
        fitfun = GLLVModels.fit_grouped_nongaussian
        p, n = 2, 8
        unit = repeat(1:4; inner = 2)
        Z = GLLVModels._grouped_incidence(unit, n)
        loads = [0.6; 0.35;;]
        @test GLLVModels._grouped_laplace_design([Z], [loads]) == sparse(kron(Z, loads))
        ...
        binomial_fit = fitfun(Ybinomial; family = GLLVModels.Binomial(), N = Nbinomial,
            terms = [term(:unit; mode = :latent, rank = 1)], unit = unit,
            iterations = 60)
        @test binomial_fit.family isa GLLVModels.Binomial
        @test isfinite(binomial_fit.loglik) || binomial_fit.loglik == -Inf

        Ybeta = [0.21 0.34 0.42 0.55 0.61 0.47 0.38 0.66]
        beta_fit = fitfun(Ybeta; family = GLLVModels.Beta(8.0, 1.0),
            terms = [term(:unit; mode = :indep, common = true)], unit = unit,
            dispersion = :shared, iterations = 60)
        @test beta_fit.family isa GLLVModels.Beta
        ...
        beta_ci = GLLVModels.grouped_nongaussian_intervals(Ybeta, beta_fit;
            unit = unit)
        ...
        nb2_fit = @test_nowarn fitfun(Ynb2; family = GLLVModels.NegativeBinomial(4.0, 0.5),
            terms = [term(:unit; mode = :indep, common = true)], unit = unit,
            dispersion = :shared, iterations = 60)
        @test nb2_fit.family isa GLLVModels.NegativeBinomial
        ...
        bad_start = [100.0, 0.0]
        @test_throws ArgumentError fitfun(Ypoisson; family = GLLVModels.Poisson(),
            terms = [term(:unit; mode = :indep, common = true)], unit = unit,
            start = bad_start)
        ...
        bridge_D = GLLVModels._trait_mean_design(1, length(unit))
        bridge_incidence = [GLLVModels._grouped_incidence(unit, length(unit))]
        bridge_objective = GLLVModels._grouped_nongaussian_objective(bridge_data,
            bridge_trials, bridge_D, bridge_terms, bridge_incidence, :poisson;
            inner_maxiter = 100, inner_tol = 1e-8)
        bridge_loads, bridge_uniques, _, _ = GLLVModels._grouped_term_unpack(
            bridge_theta[2:2], 1, bridge_terms)
        bridge_W = GLLVModels._grouped_laplace_design(bridge_incidence, bridge_loads;
            uniques = bridge_uniques)
        bridge_kernel = GLLVModels.joint_grouped_laplace_loglik(GLLVModels.Poisson(),
        ...
        structural_Z = [GLLVModels._grouped_incidence(structural_unit, structural_n),
            GLLVModels._grouped_incidence(structural_unit_obs, structural_n),
            GLLVModels._grouped_incidence(structural_cluster, structural_n),
            GLLVModels._grouped_incidence(structural_cluster2, structural_n)]
        ...
        @test GLLVModels._grouped_laplace_design(structural_Z, structural_loads;
        ...
        Ybinomial_interior = reshape([rand(binomial_rng, GLLVModels.Binomial(20, p))
            for p in binomial_probability], 1, :)
        Nbinomial_interior = fill(20, 1, length(interior_groups))
        binomial_interior = fitfun(Ybinomial_interior; family = GLLVModels.Binomial(),
        ...
        binomial_interior_ci = GLLVModels.grouped_nongaussian_intervals(Ybinomial_interior,
        ...
        Ynb2_interior = reshape([rand(nb2_rng,
            GLLVModels.NegativeBinomial(nb2_size, nb2_size / (nb2_size + mean_value)))
            for mean_value in nb2_mean], 1, :)
        nb2_interior = fitfun(Ynb2_interior;
            family = GLLVModels.NegativeBinomial(4.0, 0.5),
            terms = [term(:unit; mode = :indep, common = true)], unit = interior_groups,
            dispersion = :shared, iterations = 100)
        @test nb2_interior.converged && nb2_interior.hessian_positive_definite
        nb2_interior_ci = GLLVModels.grouped_nongaussian_intervals(Ynb2_interior,
        ...
        nb2_fixed_incidence = [GLLVModels._grouped_incidence(interior_groups,
            length(interior_groups))]
        nb2_fixed_loads, nb2_fixed_uniques, _, _ = GLLVModels._grouped_term_unpack(
            nb2_fixed_theta[2:2], 1, nb2_fixed_terms)
        nb2_fixed_W = GLLVModels._grouped_laplace_design(nb2_fixed_incidence,
            nb2_fixed_loads; uniques = nb2_fixed_uniques)
        nb2_fixed_D = GLLVModels._trait_mean_design(1, length(interior_groups))
        nb2_fixed_center = GLLVModels.joint_grouped_laplace_loglik(
            GLLVModels.NegativeBinomial(exp(nb2_fixed_theta[3]), 0.5),
            vec(Float64.(Ynb2_interior)), ones(length(interior_groups)),
            nb2_fixed_D, nb2_fixed_theta[1:1], nb2_fixed_W; link = GLLVModels.LogLink(),
        ...
        nb2_fixed_forward_result = GLLVModels.joint_grouped_laplace_loglik(
            GLLVModels.NegativeBinomial(exp(nb2_fixed_forward[3]), 0.5),
            vec(Float64.(Ynb2_interior)), ones(length(interior_groups)),
            nb2_fixed_D, nb2_fixed_forward[1:1], nb2_fixed_W; link = GLLVModels.LogLink(),
            maxiter = 100, tol = 1e-8)
        ...
        for observation in eachindex(trait_groups), trait in 1:2
            ...
            Ybeta_trait[trait, observation] = rand(beta_trait_rng,
                GLLVModels.Beta(mu * beta_phi_truth[trait], (1 - mu) * beta_phi_truth[trait]))
        end
        beta_trait_fit = fitfun(Ybeta_trait; family = GLLVModels.Beta(8.0, 1.0),
        ...
        beta_trait_ci = GLLVModels.grouped_nongaussian_intervals(Ybeta_trait, beta_trait_fit;
            unit = trait_groups)
        ...
        for observation in eachindex(trait_groups), trait in 1:2
            ...
            Ynb2_trait_boundary[trait, observation] = rand(nb2_trait_boundary_rng,
                GLLVModels.NegativeBinomial(r, r / (r + mu)))
        end
        ...
        nb2_trait_boundary_ci = GLLVModels.grouped_nongaussian_intervals(
        ...
        nb2_trait_interior_ci = GLLVModels.grouped_nongaussian_intervals(
        ...
        trait_incidence = [GLLVModels._grouped_incidence(trait_groups, length(trait_groups))]
        trait_D = GLLVModels._trait_mean_design(2, length(trait_groups))
        trait_theta = beta_trait_fit.parameters
        trait_objective = GLLVModels._grouped_nongaussian_objective(Ybeta_trait,
            ones(size(Ybeta_trait)), trait_D, trait_terms, trait_incidence, :beta;
            dispersion_mode = :trait, inner_maxiter = 100, inner_tol = 1e-8)
        trait_loads, trait_uniques, _, _ = GLLVModels._grouped_term_unpack(
            trait_theta[3:3], 2, trait_terms)
        trait_W = GLLVModels._grouped_laplace_design(trait_incidence, trait_loads;
            uniques = trait_uniques)
        trait_families = [GLLVModels.Beta(exp(trait_theta[end - 2 + trait]), 1.0)
            for trait in 1:2, observation in 1:length(trait_groups)]
        trait_kernel = GLLVModels.joint_grouped_laplace_loglik(vec(trait_families),
            vec(Ybeta_trait), ones(length(Ybeta_trait)), trait_D, trait_theta[1:2], trait_W;
            link = GLLVModels.LogitLink(), maxiter = 100, tol = 1e-8)
        ...
        marker_order = GLLVModels._grouped_nongaussian_family(
