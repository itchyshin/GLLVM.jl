...
@testset "Student-t (heavy-tailed continuous, fixed ν)" begin
    ...
    # (The earlier "implicit fit gradient" testset exercised the retired
    # marginal_loglik_laplace_aux_value_grad scalar-aux path. The fit driver now
    # optimises -studentt_marginal_loglik_laplace with a finite-diff Optim
    # gradient (matching fit_beta_gllvm / fit_gamma_gllvm); the marginal-gradient
    # FD test above already certifies that objective's gradient. An analytic
    # studentt_laplace_grad is a follow-up — see issue #105.)
    ...
    @testset "simulate → fit → recover (β, ΛΛ', σ)" begin
        ...
        σ_true = 0.8
        ν = 4.0
        # DGP: identity-link location-scale t, y = η + σ·t (inline, matching the
        # current per-family recovery tests; the engine's simulate(family, β, Λ)
        # DGP helper was retired in the refactor).
        Z = randn(K, n)
        η = β_true .+ Λ_true * Z
        Y = η .+ σ_true .* [rand(TDist(ν)) for _ in 1:p, _ in 1:n]
    ...
    @testset "simulate(fit, n) round-trips through StudentTFit" begin
        ...
        Y = η .+ σ_true .* [rand(TDist(ν)) for _ in 1:p, _ in 1:n]
        fit = fit_studentt_gllvm(Y; K = K, nu = ν)
        Y2 = simulate(fit, 20; rng = MersenneTwister(999))
        @test size(Y2) == (p, 20)
