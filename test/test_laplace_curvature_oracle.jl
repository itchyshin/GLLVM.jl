...
@testset "Laplace curvature oracle" begin
    ...
    @testset "fallback ≡ grouped analytic (Gamma/log)" begin
        ...
        for α in (0.7, 3.0, 12.0), η in (-1.5, 0.0, 2.0), y in (0.05, 1.0, 7.5)
            ...
            @test GLLVModels._glm_obs_weight(f, μ, 1, me, y, link, η) ≈
                  GLLVModels._gamma_grouped_laplace_weight(:observed, f, μ, me, y, link) rtol = 1e-10
        end
    end
    ...
    @testset "fallback ≡ grouped analytic (Beta/logit)" begin
        ...
        for φ in (8.0, 12.0, 30.0), η in (-1.2, 0.0, 0.9), y in (0.15, 0.5, 0.87)
            ...
            me = GLLVModels.mu_eta(link, η)
            @test GLLVModels._glm_obs_weight(f, μ, 1, me, y, link, η) ≈
                  GLLVModels._beta_grouped_laplace_weight(:observed, f, μ, me, y, link, η) rtol = 1e-10
        end
    end
    ...
    @testset "Beta observed curvature can be negative (PD guard is load-bearing)" begin
        ...
        μ  = GLLVModels._clamp_mu(f, GLLVModels.linkinv(link, η))
        me = GLLVModels.mu_eta(link, η)
        @test GLLVModels._glm_obs_weight(f, μ, 1, me, y, link, η) < 0
        @test GLLVModels._glm_weight(f, μ, 1, me) > 0        # Fisher is ≥ 0 by construction
    end
    ...
    @testset "Gamma/log: observed is strictly closer to quadrature" begin
        ...
        for seed in (1, 4, 7, 12)
            ...
            ef = abs(GLLVModels.gamma_marginal_loglik_laplace(Y, Λ, β, α; hessian = :fisher)   - q)
            eo = abs(GLLVModels.gamma_marginal_loglik_laplace(Y, Λ, β, α; hessian = :observed) - q)
            @test eo < ef
        end
    end
end
