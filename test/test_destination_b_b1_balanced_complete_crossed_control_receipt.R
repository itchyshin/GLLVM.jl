...
archive_path <- tempfile("b1-balanced-frozen-source-", fileext = ".tar")
tampered_archive_path <- tempfile("b1-balanced-frozen-source-tampered-", fileext = ".tar")
on.exit(unlink(c(archive_path, tampered_archive_path)), add = TRUE)
archive_status <- system2("git", c("-C", audit$frozen_provenance$source$path, "archive", "--format=tar", audit$frozen_provenance$source$git_sha), stdout = archive_path)
stopifnot(identical(archive_status, 0L))
stopifnot(identical(sha256(archive_path), audit$frozen_provenance$source$archive_sha256))
archive_bytes <- readBin(archive_path, what = "raw", n = file.info(archive_path)$size)
writeBin(c(archive_bytes, as.raw(0L)), tampered_archive_path)
stopifnot(!identical(sha256(tampered_archive_path), audit$frozen_provenance$source$archive_sha256))
