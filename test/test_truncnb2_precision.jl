...
@testset "Truncated NB2 stable mean parameterization" begin
 @testset "density and large-r limit" begin
  for r in [0.3,1.0,50.0,6.253090137749205e6,9.873510992920996e7,1e12], mu in [1e-12,0.01,1.0,4.0,20.0],y in [1,5,20]
   actual=GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(r),mu,1,y)
 ...
 @testset "moments at small means and large r" begin
  for r in [0.3,1.0,50.0,1e12],mu in [1e-12,1e-8,0.01,4.0,20.0]
   m,v=GLLVModels._truncnb2_mean_var(mu,r);mr,vr=hp_moments(mu,r)
 ...
 @testset "score and observed curvature match density derivatives" begin
  for r in [0.3,50.0,1e12],mu in [0.01,1.0,20.0],y in [1,5,20]
   f=GLLVModels.TruncatedNegBin2(r);eta=log(mu)
   ell=x->GLLVModels._glm_logpdf(f,exp(x),1,y)
   g=ForwardDiff.derivative(ell,eta)
   h=ForwardDiff.derivative(x->ForwardDiff.derivative(ell,x),eta)
   @test isapprox(g,GLLVModels._glm_score(f,mu,1,mu,y);atol=1e-9,rtol=1e-10)
   @test isapprox(-h,GLLVModels._truncnb2_observed_weight(f,mu,y,LogLink());atol=1e-9,rtol=1e-10)
  end
 end
 @test GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(4.0),1.0,1,0)==-Inf
end
...
@testset "Truncated NB2 dispersion derivatives and normalization" begin
 for r in [0.3,50.0,1e8,1e12],mu in [0.01,4.0,20.0],y in [1,5,20]
  t=log(r);f=x->GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(exp(x)),mu,1,y)
 ...
 for r in [0.3,50.0,1e12],mu in [0.01,4.0,20.0]
  probs=[exp(GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(r),mu,1,y)) for y in 1:5000]
 ...
 for y in [1000,1000000],r in [0.3,50.0,1e12],factor in [0.1,1.0,10.0]
  ...
  @test isapprox(GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(r),mu,1,y),exact;atol=2e-8,rtol=2e-12)
 end
end
...
@testset "Truncated NB2 second derivatives and formula transition" begin
 for r in [1.0,1e4,1e12],mu in [0.01,4.0,20.0],y in [1,5,20]
  ...
  eta=log(mu)
  cross=ForwardDiff.derivative(x->ForwardDiff.derivative(z->GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(exp(x)),exp(z),1,y),eta),t)
  score_derivative=ForwardDiff.derivative(x->GLLVModels._glm_score(GLLVModels.TruncatedNegBin2(exp(x)),mu,1,mu,y),t)
  @test isapprox(cross,score_derivative;atol=1e-8,rtol=1e-8)
 end
 for y in [5,20,100]
  boundary=(y-1)/(6eps(Float64)/y)^(1/6)
  @test !GLLVModels._truncnb2_logrise_series(0.999boundary,y)[2]
  @test GLLVModels._truncnb2_logrise_series(1.001boundary,y)[2]
  for r in [0.999boundary,1.001boundary]
   @test isapprox(GLLVModels._glm_logpdf(GLLVModels.TruncatedNegBin2(r),4.0,1,y),Float64(hp_logmass(y,4.0,r));atol=1e-10,rtol=1e-13)
  end
 end
end
