...
@testset "Grouped random slopes (x|g) — q>=2 correlated Σ_b" begin
    ...
    @testset "q=2 (intercept + slope) recovery" begin
        ...
        # Structural: the engine returns a valid q×q SPD Σ_b + finite loglik. Strict
        # recovery of random-SLOPE variances is genuinely boundary-prone (they collapse
        # into the residual without boundary-aware inference — the DRM-team lesson; honest
        # profile/χ̄² CIs are the SP-next follow-on). The math is gated by the q=1↔intercept
        # reduction (rtol 1e-8) + the FD-gradient below.
        @test fit.q == 2 && size(fit.Σ_b) == (2, 2)
        @test isfinite(fit.loglik)
        @test isposdef(Symmetric(fit.Σ_b))                    # valid SPD random-effect covariance
        @test all(isfinite, fit.Σ_b)                          # finite (magnitude is boundary-dependent)
        @test fit.σ_eps > 0
    end
    ...
    @testset "FD-gradient ≤ 1e-6 (q=2)" begin
        ...
        p, K, n, L = 4, 1, 60, 6
        Λ0 = 0.5 .* randn(p, K)
        grouping = vcat(collect(1:L), rand(1:L, n - L)); Z = hcat(ones(n), randn(n)); y = randn(p, n)
        codes, _ = GLLVM._code_grouping(grouping); gi = [findall(==(g), codes) for g in 1:maximum(codes)]
