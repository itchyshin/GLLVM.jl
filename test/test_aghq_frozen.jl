...
const AF = GLLVModels.ForwardDiff

@testset "AF-01 transformed Gaussian normalization and moments" begin
    m=[0.4,-0.3];S=[1.4 0.3;0.3 0.8];H=inv(S)
    a=GLLVModels.aghq_adaptation(m,H);grid=GLLVModels.aghq_grid(2,3)
    density=MvNormal(m,S)
    @test GLLVModels.aghq_frozen_logintegral(z->logpdf(density,z),a,grid) ≈ 0 atol=1e-12
...
function af_cache(beta,loading,y)
    ...
    m=GLLVModels._laplace_mode(Poisson(),[y],[1],lambda,[beta],GLLVModels.LogLink())
    H=AF.hessian(z->-af_joint(z,beta,loading,y),m)
    return GLLVModels.aghq_adaptation(m,H)
end
@testset "AF-02 observed k1 Laplace equivalence" begin
    beta=.2;loading=.7;y=3;a=af_cache(beta,loading,y)
    q=GLLVModels.aghq_frozen_logintegral(z->af_joint(z,beta,loading,y),a,GLLVModels.aghq_grid(1,1))
    ll=GLLVModels.laplace_loglik_site(Poisson(),[y],[1],reshape([loading],1,1),[beta],GLLVModels.LogLink();hessian=:observed)
    @test q ≈ ll atol=1e-10
end
@testset "AF-03 fixed-cache gradients and AF-05 chain term" begin
    theta=[.2,.7];y=3;a=af_cache(theta...,y);grid=GLLVModels.aghq_grid(1,3)
    objective=t->GLLVModels.aghq_frozen_logintegral(z->af_joint(z,t[1],t[2],y),a,grid)
    ...
    h=1e-5
    fresh=b->GLLVModels.aghq_frozen_logintegral(z->af_joint(z,b,theta[2],y),af_cache(b,theta[2],y),grid)
...
@testset "AF-04 independent quadrature and refinement" begin
    ...
    truth=log(integral)
    errors=[abs(GLLVModels.aghq_frozen_logintegral(joint,a,GLLVModels.aghq_grid(1,k))-truth) for k in [3,9,21]]
    @test errors[3]<1e-8
...
@testset "AF-06 curvature repair and AF-07 invalid inputs" begin
    ...
    for H in [[2.0 .3;.3 1.0],[-.25 0.;0. 2.],[0. 0.;0. 2.],[1e-12 0.;0. 2.],[2. .4;.2 1.]]
        ...
        a=GLLVModels.aghq_adaptation(m,H)
    ...
    a=GLLVModels.aghq_adaptation(m,Matrix{Float64}(I,2,2));m[1]=99
    @test a.mode[1]==.1
    @test_throws ArgumentError GLLVModels.aghq_adaptation([NaN],ones(1,1))
    @test_throws ArgumentError GLLVModels.aghq_adaptation([0.],reshape([Inf],1,1))
    @test_throws DimensionMismatch GLLVModels.aghq_adaptation([0.],ones(2,2))
    @test_throws ArgumentError GLLVModels.aghq_adaptation(Float64[],zeros(0,0))
    @test_throws DimensionMismatch GLLVModels.aghq_frozen_logintegral(z->-sum(abs2,z),a,GLLVModels.aghq_grid(1,3))
end
