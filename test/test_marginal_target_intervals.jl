...
@testset "Destination B marginal target intervals" begin
    ...
    result = GLLVModels._marginal_target_intervals(f, center, targets; converged=true)
    @test result.status == :available
    redundant = GLLVModels._marginal_target_intervals(f, center, targets;
    ...
    shifted=GLLVModels._marginal_target_intervals(x->f(x)+100,center,targets;converged=true)
    ...
    @test GLLVModels._marginal_target_intervals(f, center, targets; converged=false).status == :not_converged
    @test GLLVModels._marginal_target_intervals(x->GLLVModels._NLL_SENTINEL,center,targets;converged=true).status == :invalid_objective
    @test GLLVModels._marginal_target_intervals(f, center .+ .1, targets; converged=true).status == :not_stationary
    @test GLLVModels._marginal_target_intervals(x->x[1]^2, zeros(2), targets; converged=true).status == :invalid_curvature
    @test GLLVModels._marginal_target_intervals(x->-sum(abs2,x), zeros(2), targets; converged=true).status == :invalid_curvature
    boundary = [(name="zero_variance", value=x->zero(x[1]), transform=:log)]
    b = GLLVModels._marginal_target_intervals(f, center, boundary; converged=true)
    @test b.intervals[1].status == :target_unavailable
    @test isnan(b.intervals[1].lower)
    @test_throws ArgumentError GLLVModels._marginal_target_intervals(f,center,targets;converged=true,level=1.1)
end
...
@testset "Marginal interval helper on an exact Gaussian MLE" begin
    ...
    targets=[(name="mean",value=x->x[1],transform=:identity),
             (name="variance",value=x->exp(2x[2]),transform=:log)]
    ci=GLLVModels._marginal_target_intervals(objective,theta,targets;converged=true)
