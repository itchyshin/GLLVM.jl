...
@testset "precision multivariate objective failure handling" begin
    ...
    shared_failure = GLLVModels._NLL_FAIL_THRESHOLD + 1.0
    failed_arm = x -> x[1] > 0 ? shared_failure : x[1]^2

    @test !GLLVModels._pmv_valid_objective(shared_failure)
    storage = zeros(1)
    gradient = GLLVModels._pmv_fd_gradient!(storage, failed_arm, [0.0])
    @test isnan(gradient[1])
    @test !all(isfinite, GLLVModels._fd_hessian(failed_arm, [0.0]))

    targets = [(name = "theta", value = theta -> theta[1], transform = :identity)]
    marginal = GLLVModels._marginal_target_intervals(failed_arm, [0.0], targets;
        converged = true)
    ...
    @test_throws InterruptException GLLVModels._pmv_hessian_diagnostics(
