...
@testset "EM-FA solver" begin
    @testset "log-likelihood monotonically non-decreasing (EM invariant)" begin
        ...
        # Run EM with logging
        Λ, ψ, ll, n_iter, conv = GLLVModels.em_fa(y, K, tol = 1e-12, max_iter = 200)
        @test conv
        @test n_iter ≥ 1
        @test n_iter < 200
    end
    ...
    @testset "Σ_y recovery on a small FA fixture" begin
        ...
        Λ, ψ, ll, n_iter, conv = GLLVModels.em_fa(y, K, tol = 1e-10)
        @test conv
    ...
    @testset "matches LBFGS on the same fixture" begin
        ...
        Λ_em, ψ_em, ll_em, _, _ = GLLVModels.em_fa(y, K)
