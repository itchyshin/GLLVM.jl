using Test, LinearAlgebra, SparseArrays, GLLVM
...
function _db_crosscheck_precision(f; q = inv(f.scale .* f.node_covariance),
    ...
    GLLVM.PrecisionPhy(ii, jj, vv, size(q, 1), length(map),
...
@testset "Destination B builder cross-check against independent model oracle" begin
    @testset "four joint grouped sources" begin
        ...
        nll = GLLVM._grouped_gaussian_factor_nll(Y, beta, incidences, loadings,
        ...
        wrong_nll = GLLVM._grouped_gaussian_factor_nll(Y, beta,
    ...
    @testset "retained unused ancestor precision consumer" begin
        ...
        got = GLLVM.multivariate_phylo_precision_loglik(y, pp, f.trait_loadings,
        ...
        wrong_scale = GLLVM.multivariate_phylo_precision_loglik(y, double_scaled, f.trait_loadings,
            f.residual_sd .^ 2; sigma2_phy = 1.0, species_id = f.observation_nodes)
        @test !isapprox(wrong_scale, got; atol = 1e-7, rtol = 1e-7)

        wrong_det = _db_crosscheck_precision(f; q = pp.Q, log_det = pp.log_det + log(1.7))
        @test !isapprox(GLLVM.multivariate_phylo_precision_loglik(y, wrong_det, f.trait_loadings,
        ...
        @test !isapprox(GLLVM.multivariate_phylo_precision_loglik(y, swapped, f.trait_loadings,
            f.residual_sd .^ 2; sigma2_phy = 1.0, species_id = f.observation_nodes), got;
            atol = 1e-7, rtol = 1e-7)
    end
end
