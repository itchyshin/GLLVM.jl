<!-- LOOP binding copy — frozen at G0 approval 2026-08-09.
     Canonical path also: docs/dev-log/plans/2026-08-09-zip-x-engine-arc0-ultra-plan.md -->
...
```mermaid
flowchart TD
  gate["Merge gate: #197+#198 on main"] --> S0["S0 Orient + twin re-cite + fresh wt"]
  S0 --> S1["S1 Call-site map twopart + offset + bridge"]
  S1 --> S2["S2 ZIPCovFit + fit_zip_gllvm_cov"]
  S2 --> S3["S3 Identity + FD tests"]
  S3 --> S4["S4 Bridge + formula + capabilities"]
  S4 --> S5["S5 Docs/board/check-log"]
  S5 --> S6["S6 Mech-verify tallies"]
  S6 --> S7["S7 Rose claim gate + after-task"]
  S7 --> S8["S8 Melissa reconcile + STOP"]
```
