...
**Recommended / actual:** 210–260 / ~resume session after merge gate ·
**Requested / used:** N/A / N/A · **Rungs/cohorts completed:** Arc 0 only
(Rung 2 not taken)  
**Under-run event:** none (Rung 2 left deferred per Q1)  
**Calibration:** dual-γ reused existing `offsetz`/`offsetc`; no substrate
repair; closest analogue Ordinal/Gamma+X  
**Metric movement:** no ZIP+X fitter → `fit_zip_gllvm_cov` + bridge admit
(still no twin Δ)  
**Result:** DoD met (Julia claim). **STOP** · **Next arc:** ZIP+X confint
under X **or** twin-ZIP Identity re-check — fresh `/arc-creation`
