...
|---|---|---|---|---|
| **Destination B (frozen-0.7.0) G1/G2 closeout** | **PR OPEN / READY — STOP, not merged (real CI red, not a merge conflict)** | `codex/destination-b-b1-integration-20260910` @ `a4ba13ea` → PR [#318](https://github.com/itchyshin/GLLVM.jl/pull/318) | after-task `docs/dev-log/after-task/2026-09-13-destination-b-beyond-slice.md`, PR #318 comment `2026-09-13` | **Update (2026-09-13, later same day): the git-dirty blocker below is RESOLVED, not still open.** `git merge origin/main` (6 commits, all logo/asset) resolved with one trivial append-log conflict in `check-log.md` (concatenated, no content dropped); merge commit `a4ba13ea` pushed non-force; GitHub now reports `mergeable: MERGEABLE`; PR marked ready-for-review. This triggered the branch's **first-ever CI run** — and it is genuinely red: 7 of 10 checks failed (Julia 1 shards 1–3, Julia 1.10 shards 1–3, advisory Frozen-R smoke), spanning real errors (not just assertion mismatches) in `test_b1_fixed_point_marginal_curvature_protocol.jl`, `test_destination_b_phylo_uncertainty.jl`, `test_grouped_nongaussian_fit.jl`, `test_destination_b_a4_s4_tree_julia_own_optimum.jl`, `test_destination_b_b1_joint_gaussian_paired_fit.jl`, `test_precision_multivariate_fit.jl`, `test_destination_b_grouping_interval_matrix.jl`, `test_destination_b_joint_other_families.jl` (e.g. NB2-log joint fit `hessian_positive_definite == false`). One unrelated `test_cv.jl:145` flake also fails independently on `main`'s own tip today — not attributable to this branch. Full evidence in the PR #318 comment. **This is now a genuine engine-correctness blocker across ~8 new test files, not a git-mechanics or CI-plumbing problem** — out of scope for a merge-only task; needs either a dedicated fix arc or a maintainer decision to split the 5 safe closeout docs into a fresh clean PR. **≠ B1 retry/redesign ≠ S4 probe ≠ merge-on-trust ≠ claim DestB done ≠ tolerance-widen the new failures.** |
