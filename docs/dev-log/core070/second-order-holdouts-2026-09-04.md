...
| Student-t ν | Nonlinear boundary; Wald SE pathology (panel finding 4) | Free ν OUT; **fixed-ν `StudentTFit` ∈ `_FamilyFit` 2026-09-15** |
...
**Summary:** OUT **6** · PARTIAL **6** · NOT ATTEMPTED **1** (plus batch-1 NB2 noted below; Multinomial FE + Student-t fixed-ν now PARTIAL)
...
| Student-t ν (free) | OUT | Wald SE pathology at ν boundary (§3); estimated-ν `_family_ci` throws; no SO pairing | Contract §3 lines 120–123; `src/confint_family.jl` `_family_ci(::StudentTFit)` |
