...
| Lognormal, Truncated-Poisson, Truncated-NB2 | No Wald `_CIFit` for Truncated*; **LognormalFit ∈ `_FamilyFit` 2026-09-15** | Truncated API gap; Lognormal native Wald wired (bridge CI guard still held for #357) |
...
| Truncated-Poisson, Truncated-NB2 | OUT (SO) / PARTIAL (1st) | `TruncatedPoissonFit`, `TruncatedNegBin2Fit` ∉ `_CIFit`; logLik parity fixtures only | `src/confint_family.jl:31-45`; parity tests |
