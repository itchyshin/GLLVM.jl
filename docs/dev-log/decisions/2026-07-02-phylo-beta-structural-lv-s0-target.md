# Phylo x Beta Structural LV S0 Target
...
Scope: fifth candidate non-Gaussian structural-source LV target after phylo x Poisson, Binomial, NB2, and Gamma

## Decision

Use phylo x Beta logit-link as the next structural-source candidate after the
Poisson, Binomial, NB2, and Gamma S1 route proofs. This page names the model,
estimand, precision treatment, and local anchors before any Totoro diagnostic,
DRAC run, R grammar exposure, bridge promotion, or support claim.

2026-07-02 follow-up: the Beta-specific internal S1 likelihood proof and one
...
The proof shows that the combined likelihood reduces to ordinary Beta `X_lv`
as `sigma_phy^2 -> 0`, reduces to
`phylo_glm_marginal_loglik(Beta())` as `Lambda -> 0`, matches a dense
leaf-covariance reference, and guards strictly interior responses. This
...
The shared Beta precision `phi` is a nuisance parameter estimated jointly in
S1; it is not `alpha_lv` and it is not the interval target. The canary records
a loose interior `phi` guard so the route is not mistaken for a boundary
precision story.
...
`a_t` and the shared Beta precision `phi` are nuisance components and are
...
| `phi` | shared Beta precision | fixed positive scalar in DGP; fitted nuisance in S1 | `phi_ok` loose interior guard | nuisance, not interval target |
...
| `eta[t,s]` | combined logit predictor | `beta[t] + Lambda[t,:]'z_s + a_t` | independent dense/sparse check | noiseless link-scale surface |
| `Y[t,s]` | Beta logit family | `Beta(mu[t,s] * phi, (1 - mu[t,s]) * phi)` | strictly interior response matrix | observed continuous responses in `(0,1)` |
...
The private combined Beta route now proves:

1. `sigma_phy^2 -> 0` reduction to ordinary Beta `X_lv`;
2. `Lambda = 0` reduction to `phylo_glm_marginal_loglik(Beta())`;
3. dense/sparse equality for the phylo random-effect component;
4. guards for finite strictly interior responses and positive finite `phi`;
5. one stochastic selected-entry `B_eta_realized` profile-LR canary with finite
   ...
   LR below the one-df 95 percent cutoff, and an interior `phi` guard.

## Exclusions

- no source-specific R `phylo_latent(..., lv = ~ env)` grammar exposure;
- no public `fit_phylo_beta_xlv` or `confint_lv_effects` route;
- no bridge profile/bootstrap transport;
- no coverage calibration;
- no bootstrap rescue;
- no transfer to Ordinal, spatial, animal, kernel, mixed-family vectors, masks,
  missing responses, or `unique=` parity.

## Rose Verdict

Rose verdict: PASS WITH NOTES - phylo x Beta is symbolically aligned and now
