...
| --- | --- | --- |
| Ordinary `latent(..., lv = ~ env)` | Covered as the ordinary score-mean grammar; Julia bridge has ordinary `predictor_informed_lv` point rows for admitted one-part families. | `gllvmTMB` ordinary grammar; `GLLVM.bridge_capabilities()` lists ordinary predictor-informed LV rows for Gaussian, Poisson, Binomial links, NB2, Beta, and Gamma. |
