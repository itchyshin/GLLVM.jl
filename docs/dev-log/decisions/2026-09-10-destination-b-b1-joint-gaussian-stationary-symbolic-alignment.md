# Destination B B1 — fixed exploratory four-source Gaussian candidate

**Status:** retrospective exploratory record, corrected on 2026-09-10. The
first commit containing this design and receipt (`fcd67a2f`) was made after the
frozen-R execution, and no immutable pre-run timestamp, protocol hash, or
commit was retained. Therefore this record cannot establish temporal
pre-registration. It does not replace or alter the retained p=2, n=36
non-stationary diagnostic.
...
- Fixed true scales: latent loadings `(0.72, -0.51)`; nested-observation SDs
  ...
  residual SD `0.17`.
- The one run used `n_init = 1`, `se = FALSE`, and the recorded tightened
  `nlminb` control only. It has no retained evidence of a post-hoc restart,
  warm start, data change, seed search, or acceptance-tolerance change; this
  is not a claim that those settings were immutably fixed before execution.
...
The single exploratory frozen-R run completed in 0.77 seconds but did not meet
the gate: `nlminb` returned `singular convergence (7)` and the named source
gradient maximum was `9.9329370235209566e-5`, about 99 times the fixed
threshold. The immutable receipt is therefore a negative exploratory-candidate
...
blocker is a separately pre-registered, identification-based remedy with an
immutable pre-run protocol hash, timestamp, or commit that can produce source
convergence code zero and maximum source gradient at most `1e-6`; it must not
reuse this receipt name, alter this fixture after seeing the result, or relax
the threshold.
