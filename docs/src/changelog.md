...
- **PARTIAL:** predictor-informed latent-score means for the ordinary unit-tier
  ...
  `fit_gamma_gllvm(...; X_lv=...)`, and `fit_binomial_gllvm(...; X_lv=...)` for
  complete-response Gaussian, Poisson (log link), shared-dispersion NB2,
  shared-shape Gamma, and binomial logit/probit/cloglog point fits. `getLV(...;
