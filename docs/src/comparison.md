...
- **R `gllvmTMB`** — fits the GLLVModels exactly. The reference engine for
  every grid cell. TMB Laplace + L-BFGS in C++.
- **GLLVModels.jl** — fits the same GLLVModels. Closed-form Gaussian marginal +
  `Optim` L-BFGS + `ForwardDiff` gradients, with a PPCA closed-form
  warm start for the no-other-RE path.
- **MixedModels.jl** — cannot fit a `K ≥ 1` GLLVModels directly because it
  has no latent-factor block in the formula DSL. On `K = 0` cells the
  GLLVModels degenerates to an LMM and the three engines coincide (the
  ...
  strictly easier problem than the GLLVModels, with no rank-`K` loading
...
head-to-head on the GLLVModels.

## Headline numbers

From PERF+E (full numbers in the benchmark report):

- **GLLVModels.jl vs R `gllvmTMB` on the GLLVModels**: median speedup of
...
- **GLLVModels.jl vs MixedModels.jl on the proxy LMM**: MixedModels.jl is
  ...
  expensive part of the GLLVModels evaluation.
...
- MixedModels.jl's logLik on each row is on a *different* model from
  the GLLVModels logLik. It is not directly comparable unless `K = 0`. It
