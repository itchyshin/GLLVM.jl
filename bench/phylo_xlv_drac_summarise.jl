...
function fmt_optional_int_set(xs)
    vals = sort(unique(x for x in xs if x !== nothing))
...
function summarise(rows)
    ...
    println("| scenario | lambda | p | n_sites | K | target | method | tasks | fit ok | usable entries | mean coverage (MCSE) | entry coverage | RMSE mean | fit sec mean | CI sec mean | boot n | boot iter cap | bootstrap ok | CI status |")
    println("|---|---:|---:|---:|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for key in sort(collect(keys(groups)))
        ...
        boot_requested = [parse_optional_int(get(r, "n_boot", "")) for r in rs]
        boot_iterations = [parse_optional_int(get(r, "bootstrap_iterations", "")) for r in rs]
        ...
        @printf("| %s | %s | %s | %s | %s | %s | %s | %d | %d | %d | %s (%s) | %s | %s | %s | %s | %s | %s | %s | %s |\n",
                ...
                fmt(ci_seconds_mean), fmt_optional_int_set(boot_requested),
                fmt_optional_int_set(boot_iterations),
                fmt_optional_int(boot_total), join(statuses, ";"))
    end
end
