#!/usr/bin/env bash
# T4 NB2 pre-run — Totoro launcher (cell 3/3, gated after Poisson G2 PASS).
#
# Authority: docs/dev-log/plans/2026-09-05-totoro-t4-prerun-programme.md
# Cell 3 of 3: nb2 p=20 n=500 K=2 seed=42 (each-own-optimum, not gated).
# Watch: NB2 boundary dispersion can NaN Julia vcov while R block-tolerates.
#
# Default: print rsync + ssh commands and exit 0.
# Launch only when GLLVM_TOTORO_LAUNCH=1 is set explicitly in the shell.
#
# Usage:
#   tools/t4_totoro_nb2_prerun.sh              # dry-run (default)
#   GLLVM_TOTORO_LAUNCH=1 tools/t4_totoro_nb2_prerun.sh
...
echo "=== T4 NB2 pre-run — Totoro queue spec ==="
...
echo "Risk: NB2 boundary dispersion — Julia vcov may NaN; record honest FAIL if triggered."
echo ""
