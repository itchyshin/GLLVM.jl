...
STATE=ROOT/'.unlazy/core070-aghq/truncnb2-bfgs-02'
...
def checks(r):
    ...
    assert r['optimizer']=='optim_BFGS_reltol_1e-12'
    assert math.isclose(r['samepoint_delta'],r['samepoint_native_nll']-r['r_objective'],rel_tol=0,abs_tol=1e-12)
    return dict(samepoint_density=math.isfinite(r['samepoint_delta']) and abs(r['samepoint_delta'])<=1e-6,
      reported_objective=abs(r['r_objective']+r['r_loglik'])<=1e-8,native_converged=r['native_converged'],r_code=r['r_code']==0,
...
def retained_failure():
    state=ROOT/'.unlazy/core070-aghq/truncnb2-bfgs-01'
    ...
    assert receipt['environment_overrides']==plan['env'] and receipt['supervisor_error'] is None
    assert receipt['status']=='FAIL' and [r['exit_code'] for r in receipt['results']]==[0,1,0]
    assert [r['id'] for r in receipt['results']]==['oracle-before','truncnb2-bfgs','oracle-after']
    ...
    assert 'ParseError' in (state/'attempt1/process/01.log').read_text()
...
def verify(readback_only=False):
    ...
    assert [x['id'] for x in process['results']]==['oracle-before','truncnb2-bfgs','oracle-after']
    ...
    changes=[('data_sha256','0'*64),('fixture_sha256','0'*64),('dgp_sha256','0'*64),
      ('native_gradient_max',-1),('r_gradient_max',-1),('fd_stability',-1),('loglik_delta',-1),('samepoint_delta',-1)]
    ...
    if readback_only: print('TRUNCNB2_BFGS_READBACK_PASS');return
    assert not failed,'TRUNCNB2_REPLAY_UNQUALIFIED: '+','.join(failed)
    print('ORIGINAL_TRUNCNB2_BFGS_QUALIFIED')
