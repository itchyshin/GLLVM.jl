...
import core070_verify_truncnb2_required as gate

original=gate.STATE
mutations=[
 ('missing_run',lambda d:(d/'attempt1/receipts/run.toml').unlink()),
 ('omitted_policy',lambda d:(d/'attempt1/receipts/truncnb2-policy.toml').unlink()),
 ('altered_policy',lambda d:(d/'attempt1/receipts/truncnb2-policy.toml').write_text('policy="default"\n')),
 ('altered_raw_fit',lambda d:(d/'attempt1/receipts/truncnb2-whole-fits.rds').write_bytes(b'wrong fit')),
...
print('TRUNCNB2_REQUIRED_ARTIFACT_NEGATIVES_PASS',len(mutations))
