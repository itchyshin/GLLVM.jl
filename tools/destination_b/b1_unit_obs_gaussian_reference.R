#!/usr/bin/env Rscript
# Frozen-R source-alignment receipt for one nested, replicated Gaussian W-tier fit.
# It does not edit, rebuild, or otherwise alter the gllvmTMB reference source.

usage <- function(status = 0L) {
  cat("Usage: b1_unit_obs_gaussian_reference.R --source PATH --library PATH --output PATH\n")
  quit(status = status)
}
...
archive <- tempfile("gllvmTMB-frozen-r070-unit-obs-", fileext = ".tar")
...
set.seed(20260910L)
n_trait <- 2L; n_unit <- 12L; n_obs_per_unit <- 2L; n_replicate <- 2L
trait_levels <- paste0("trait_", seq_len(n_trait))
unit_levels <- paste0("unit_", seq_len(n_unit))
rows <- do.call(rbind, lapply(unit_levels, function(unit_value) {
  do.call(rbind, lapply(seq_len(n_obs_per_unit), function(obs_index) {
    data.frame(unit = unit_value, obs = paste(unit_value, "obs", obs_index, sep = "__"),
...
obs_levels <- unique(rows$obs)
trait_mean <- c(-0.35, 0.20)
trait_sd_w <- c(0.65, 0.30)
w <- rbind(rnorm(length(obs_levels), sd = trait_sd_w[1L]),
           rnorm(length(obs_levels), sd = trait_sd_w[2L]))
rows$value <- trait_mean[match(rows$trait, trait_levels)] +
  w[cbind(match(rows$trait, trait_levels), match(rows$obs, obs_levels))] +
  rnorm(nrow(rows), sd = 0.22)
data <- data.frame(value = rows$value,
  trait = factor(rows$trait, levels = trait_levels),
  unit = factor(rows$unit, levels = unit_levels),
  obs = factor(rows$obs, levels = obs_levels),
  replicate = rows$replicate)
data_csv <- tempfile("b1-unit-obs-gaussian-data-", fileext = ".csv")
utils::write.csv(data, data_csv, row.names = FALSE)
data_md5 <- unname(tools::md5sum(data_csv)); unlink(data_csv)

formula_text <- "value ~ 0 + trait + indep(0 + trait | obs)"
receipt <- list(
  ...
  limitations = c("Not Julia parity until independently checked by Julia.",
    ...
    "Nested diagonal unit_obs only; crossed and latent forms are excluded."),
  ...
  specification = list(formula = formula_text, family = "gaussian()", trait = "trait",
    unit = "unit", unit_obs = "obs", REML = FALSE, control = list(n_init = 1L, se = FALSE),
    n_trait = n_trait, n_unit = n_unit, n_obs_per_unit = n_obs_per_unit,
    n_replicate = n_replicate, n_observation = n_unit * n_obs_per_unit * n_replicate,
    row_order = "unit, nested obs, replicate, trait", seed = 20260910L, data_md5 = data_md5),
  response_long = list(value = as_receipt_value(data$value), trait = as.character(data$trait),
    unit = as.character(data$unit), obs = as.character(data$obs), replicate = data$replicate),
  mapping = NULL, fit = NULL, failure = list(present = FALSE)
)

started <- Sys.time()
fit <- tryCatch(gllvmTMB::gllvmTMB(
  value ~ 0 + trait + indep(0 + trait | obs), data = data, family = gaussian(),
  trait = "trait", unit = "unit", unit_obs = "obs", REML = FALSE,
...
if (inherits(fit, "error")) {
  ...
  required <- c("b_fix", "log_sigma_eps", "theta_diag_W")
  if (!is.list(fitted) || any(vapply(required, function(name) is.null(fitted[[name]]), logical(1L)))) {
    stop("Frozen-R fit did not expose required b_fix/log_sigma_eps/theta_diag_W coordinates.")
  }
  sigma_w <- tryCatch(gllvmTMB::extract_Sigma(fit, level = "unit_obs", part = "total")$Sigma,
    error = function(e) NULL)
  is.null(sigma_w) && stop("Frozen-R canonical unit_obs Sigma extraction failed.")
  ...
  receipt$mapping <- list(trait_id = field(tmb_data, "trait_id"), unit_id = field(tmb_data, "site_id"),
    unit_obs_id = field(tmb_data, "site_species_id"), n_units = field(tmb_data, "n_sites"),
    n_unit_obs = field(tmb_data, "n_site_species"))
  receipt$fit <- list(fit_seconds = fit_seconds, logLik = as.numeric(stats::logLik(fit)),
    ...
    theta_diag_W = as_receipt_value(fitted$theta_diag_W),
    log_sd_W = as_receipt_value(log(field(report, "sd_W"))), Sigma_W = as_receipt_value(sigma_w),
    sigma_eps = sigma_eps)
}
...
message("Frozen-R unit_obs source-alignment receipt written: ", output_path)
