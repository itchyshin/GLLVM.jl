...
const _DB_TREE_DLL_SHA = "91bfa6d90fbf3e4f42e1f4160583f2607f51a7839bb54a02a209da6e31a59beb"

"""Pure validation and cross-evaluation, not optimizer/admission qualification."""
function _db_check_tree_gaussian(doc, reference)
    require(ok, message) = ok || throw(ArgumentError(message))
    require(doc["schema_version"] == "destination-b-tree-gaussian-marginal-1", "wrong schema")
    ...
    require(doc["dll_sha256"] == _DB_TREE_DLL_SHA, "wrong frozen DLL")
    require(doc["precision_reference_sha256"] == _DB_TREE_PRECISION_SHA, "wrong precision reference hash")
    require(doc["precision"] == reference, "embedded precision differs from retained reference")
    require(doc["input_route"] == "tree" && doc["residual_mode"] == "shared" &&
        doc["rank"] == 1 && doc["unique"] === false, "different model")
    require(doc["formula"] == "value ~ 0 + trait + phylo_latent(species, d=1, tree=tree, unique=FALSE)", "different formula")
    ...
    observed = Int.(reference["species_node_one_based"])
    labels = String.(reference["node_labels"])
...
function compare_tree_gaussian_reference(path, precision_path, dll_path, receipt_path)
    ...
    try
        bytes2hex(sha256(read(precision_path))) == _DB_TREE_PRECISION_SHA || throw(ArgumentError("wrong precision file"))
        bytes2hex(sha256(read(dll_path))) == _DB_TREE_DLL_SHA || throw(ArgumentError("wrong DLL file"))
        ...
        checked = _db_check_tree_gaussian(doc,reference)
        merge!(receipt,Dict("status"=>"pass","comparisons"=>checked.comparisons,
            ...
            "precision_file_sha256"=>_DB_TREE_PRECISION_SHA,"dll_sha256"=>_DB_TREE_DLL_SHA,
...
if abspath(PROGRAM_FILE)==abspath(@__FILE__)
    length(ARGS)==4 || error("usage: compare_tree_gaussian_reference.jl R_JSON PRECISION_JSON DLL RECEIPT")
    JSON3.write(stdout,compare_tree_gaussian_reference(ARGS...))
    println()
end
