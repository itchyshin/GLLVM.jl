...
const _DB_PEDIGREE_FIT_REFERENCE_SHA = "b8d5f2e884720ad86f3a0d0d562ec5b25248113d0e641825803e4d5cee8e5a06"

"""
Record one public independent Julia pedigree fit and all interval diagnostics.
The R optimizer's status is retained separately and never upgraded by Julia.
"""
function fit_pedigree_gaussian_reference(path,precision_path,dll_path,receipt_path)
    ...
    receipt = _dbfit_new_receipt(path,dll_path,_DB_PEDIGREE_DLL_SHA)
    receipt["expected_reference_file_sha256"] = _DB_PEDIGREE_FIT_REFERENCE_SHA
    ...
    receipt["checker_source_sha256"] = bytes2hex(sha256(read(joinpath(@__DIR__,"compare_pedigree_gaussian_reference.jl"))))
    try
        receipt["reference_file_sha256"] = bytes2hex(sha256(read(path)))
        receipt["reference_file_sha256"] == _DB_PEDIGREE_FIT_REFERENCE_SHA || throw(ArgumentError("wrong immutable R reference"))
        input = compare_pedigree_gaussian_reference(path,precision_path,dll_path,input_path)
        ...
        checked = _db_check_pedigree_gaussian(doc,precision)
        ...
        receipt["route"] = Dict("entrypoint"=>"GLLVModels.fit_gllvm","family"=>"Normal",
            ...
            "n_aug"=>12,"n_observed"=>8,"retained_unobserved_founders"=>4)
...
if abspath(PROGRAM_FILE)==abspath(@__FILE__)
    length(ARGS)==4 || error("usage: fit_pedigree_gaussian_reference.jl R_JSON PRECISION_JSON DLL RECEIPT")
    JSON3.write(stdout,fit_pedigree_gaussian_reference(ARGS...))
    println()
end
