#!/usr/bin/env Rscript
# Frozen-R source-alignment receipt for one crossed, replicated Gaussian cluster-tier fit.
# It does not edit, rebuild, or otherwise alter the gllvmTMB reference source.

usage <- function(status = 0L) {
  cat("Usage: b1_cluster_gaussian_reference.R --source PATH --library PATH --output PATH\n")
  quit(status = status)
}
...
archive <- tempfile("gllvmTMB-frozen-r070-cluster-", fileext = ".tar")
...
set.seed(20260911L)
n_trait <- 2L; n_cluster <- 6L; n_unit_per_cluster <- 4L; n_replicate <- 2L
trait_levels <- paste0("trait_", seq_len(n_trait))
cluster_levels <- paste0("cluster_", seq_len(n_cluster))
rows <- do.call(rbind, lapply(cluster_levels, function(cluster_value) {
  do.call(rbind, lapply(seq_len(n_unit_per_cluster), function(unit_index) {
    data.frame(cluster_id = cluster_value,
...
trait_mean <- c(-0.30, 0.18)
trait_sd_cluster <- c(0.60, 0.28)
q <- rbind(rnorm(n_cluster, sd = trait_sd_cluster[1L]),
           rnorm(n_cluster, sd = trait_sd_cluster[2L]))
rows$value <- trait_mean[match(rows$trait, trait_levels)] +
  q[cbind(match(rows$trait, trait_levels), match(rows$cluster_id, cluster_levels))] +
  rnorm(nrow(rows), sd = 0.21)
data <- data.frame(value = rows$value,
  trait = factor(rows$trait, levels = trait_levels),
  unit = factor(rows$unit),
  cluster_id = factor(rows$cluster_id, levels = cluster_levels),
  replicate = rows$replicate)
data_csv <- tempfile("b1-cluster-gaussian-data-", fileext = ".csv")
utils::write.csv(data, data_csv, row.names = FALSE)
data_md5 <- unname(tools::md5sum(data_csv)); unlink(data_csv)

formula_text <- "value ~ 0 + trait + indep(0 + trait | cluster_id)"
receipt <- list(
  ...
  limitations = c("Not Julia parity until independently checked by Julia.",
    ...
    "Crossed diagonal cluster only; latent and full forms are excluded."),
  ...
  specification = list(formula = formula_text, family = "gaussian()", trait = "trait",
    unit = "unit", cluster = "cluster_id", REML = FALSE, control = list(n_init = 1L, se = FALSE),
    n_trait = n_trait, n_cluster = n_cluster, n_unit_per_cluster = n_unit_per_cluster,
    n_replicate = n_replicate, n_observation = n_cluster * n_unit_per_cluster * n_replicate,
    row_order = "cluster, unit, replicate, trait", seed = 20260911L, data_md5 = data_md5),
  response_long = list(value = as_receipt_value(data$value), trait = as.character(data$trait),
    unit = as.character(data$unit), cluster_id = as.character(data$cluster_id), replicate = data$replicate),
  mapping = NULL, fit = NULL, failure = list(present = FALSE)
)

started <- Sys.time()
fit <- tryCatch(gllvmTMB::gllvmTMB(
  value ~ 0 + trait + indep(0 + trait | cluster_id), data = data, family = gaussian(),
  trait = "trait", unit = "unit", cluster = "cluster_id", REML = FALSE,
...
if (inherits(fit, "error")) {
  ...
  required <- c("b_fix", "log_sigma_eps", "theta_diag_species")
  if (!is.list(fitted) || any(vapply(required, function(name) is.null(fitted[[name]]), logical(1L)))) {
    stop("Frozen-R fit did not expose required b_fix/log_sigma_eps/theta_diag_species coordinates.")
  }
  sigma_w <- tryCatch(gllvmTMB::extract_Sigma(fit, level = "cluster", part = "total")$Sigma,
    error = function(e) NULL)
  is.null(sigma_w) && stop("Frozen-R canonical cluster Sigma extraction failed.")
  sigma_eps <- field(report, "sigma_eps")
  is.null(sigma_eps) && stop("Frozen-R report did not expose sigma_eps.")
  receipt$mapping <- list(trait_id = field(tmb_data, "trait_id"), unit_id = field(tmb_data, "site_id"),
    cluster_id = field(tmb_data, "species_id"), n_units = field(tmb_data, "n_sites"),
    n_cluster = field(tmb_data, "n_species"))
  receipt$fit <- list(fit_seconds = fit_seconds, logLik = as.numeric(stats::logLik(fit)),
    ...
    theta_diag_species = as_receipt_value(fitted$theta_diag_species),
    log_sd_cluster = as_receipt_value(log(field(report, "sd_q"))), Sigma_cluster = as_receipt_value(sigma_w),
    sigma_eps = sigma_eps)
}
...
message("Frozen-R cluster source-alignment receipt written: ", output_path)
