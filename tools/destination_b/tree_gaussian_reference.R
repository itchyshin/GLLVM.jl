#!/usr/bin/env Rscript
# Exact frozen-R Gaussian height-four phylo_latent pilot. No R package source is edited.
args <- commandArgs(trailingOnly = TRUE)
if (!(length(args) %in% c(3L, 4L))) stop("usage: tree_gaussian_reference.R PRIVATE_LIBRARY PRECISION_JSON OUTPUT_JSON [nlminb|bfgs]")
...
expected_precision <- "ab01ee47206565eb1af7da391af313953b8a97c6bb11d9b6160655aaa0429170"
...
dimnames(Q) <- list(precision$node_labels, precision$node_labels)
observed <- precision$species_node_one_based
tips <- precision$observed_labels
tree <- ape::read.tree(text = precision$newick)
traits <- c("a", "b", "c")
seed <- 20260907L
set.seed(seed)
...
genetic <- as.numeric(t(chol(solve(as.matrix(Q)))) %*% rnorm(14L))
beta <- c(.4, -.2, .3)
loading <- c(.9, .5, -.6)
...
receipt <- list(schema_version = "destination-b-tree-gaussian-marginal-1",
  ...
  formula = "value ~ 0 + trait + phylo_latent(species, d=1, tree=tree, unique=FALSE)",
  input_route = "tree",
...
tryCatch({
  ...
  fit <- withCallingHandlers(gllvmTMB(
    value ~ 0 + trait + phylo_latent(species, d = 1, tree = tree, unique = FALSE),
  ...
  stopifnot(identical(engineQ, as.matrix(Q)), fit$tmb_data$n_aug_phy == 14L,
  ...
  cat("FROZEN_TREE_GAUSSIAN_REFERENCE_RECORDED\n")
  cat(sha(output), "\n")
}, error = function(e) {
