#!/usr/bin/env julia
# M2 remainder: Beta-logit each-own-optimum 2SO smoke vs signed contract §4.
#
# Usage:
#   julia --project=. tools/core070_second_order/smoke_beta_eoo.jl [output.json]

using GLLVM, RCall, Dates, Random, LinearAlgebra, ForwardDiff

const OUT_DEFAULT = joinpath(@__DIR__, "..", "..", "docs", "dev-log", "core070",
    "beta-2so-eoo-smoke-receipt-2026-09-06.json")
...
t0 = time()
cell = run_one_cell("beta_logit")
wall = time() - t0

pass, issues = assess_eoo(cell)
receipt = merge(cell, eoo_receipt_extras("beta_logit", "beta-2so-eoo-smoke-m2-remainder"),
    Dict{String,Any}(
        "eoo_smoke_pass" => pass,
...
println("BETA_2SO_EOO_SMOKE $status ($(round(wall; digits=1))s)")
for iss in issues
    println("  issue: $iss")
end
exit(pass ? 0 : 1)
