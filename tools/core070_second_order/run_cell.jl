...
#
# DGP snippets below are PORTED VERBATIM (seeds, coefficients, loadings)
# from the already-verified paired fixtures in test/parity/*.jl (read-only
# source; nothing there was edited). Provenance is named per cell in the
# receipt's `dgp_source` field.
#
# Convention (frozen by the orchestrator, docs/dev-log/core070/
# second-order-parity-contract.md §2): observed joint Hessian on both sides.
# R: gllvmTMBcontrol(n_init=1L, se=TRUE), sd_report$par.fixed / cov.fixed
# read directly (matches gllvmTMB:::vcov.gllvmTMB_multi's b_fix rows).
# Julia: confint(fit, Y; method=:wald, ...) for the per-term SE/pd_hessian/
# boundary_terms (T14 fields), plus a private-API rebuild of the SAME
# marginal NLL (GLLVM._family_ci / GLLVM._fd_hessian, exactly the technique
# the 5-cell pre-run used for poisson/binomial/beta/nb2) to get the FULL
# fixed-effect covariance block for the Frobenius comparison.
...
# ---------------------------------------------------------------------------
# Ported helpers (test/parity/parity_helpers.jl -- read-only source, not
# edited; these three are pure-Julia, no RCall).
# ---------------------------------------------------------------------------
