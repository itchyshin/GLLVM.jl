#!/usr/bin/env julia
# M2 Foundation day-1: Gaussian each-own-optimum 2SO smoke vs signed contract §4.
#
# Usage:
#   julia --project=. tools/core070_second_order/smoke_gaussian_eoo.jl [output.json]
...
const OUT_DEFAULT = joinpath(@__DIR__, "..", "..", "docs", "dev-log", "core070",
    "gaussian-2so-eoo-smoke-receipt-2026-09-05.json")
...
t0 = time()
cell = run_one_cell("gaussian")
wall = time() - t0

pass, issues = assess_eoo(cell)
receipt = merge(cell, Dict{String,Any}(
    "receipt_id" => "gaussian-2so-eoo-smoke-m2-day1",
...
println("GAUSSIAN_2SO_EOO_SMOKE $status ($(round(wall; digits=1))s)")
for iss in issues
    println("  issue: $iss")
end
exit(pass ? 0 : 1)
