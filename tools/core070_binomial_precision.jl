# Uniform public-control refinement; keep all original default results intact.
...
"""
ll = rcopy(Float64,R"as.numeric(logLik(refined_fit))")
gradient = rcopy(Vector{Float64},R"as.numeric(refined_gradient)")
code = rcopy(Int,R"as.integer(refined_fit$opt$convergence)")
checks = Dict(
 "baseline_likelihood_reproduced"=>original.logLik==baseline["r_loglik"],
 "baseline_gradient_reproduced"=>rcopy(Float64,R"max(abs(original_gradient))")==baseline["r_gradient_max"],
 "refined_converged"=>code==0,
 "refined_finite"=>isfinite(ll)&&all(isfinite,gradient)&&rcopy(Bool,R"all(is.finite(refined_fit$opt$par))"),
 "refined_gradient"=>maximum(abs,gradient)<=1e-4,
 "same_native_likelihood"=>isapprox(ll,baseline["native_loglik"];rtol=1e-6,atol=0),
 "refined_objective"=>abs(ll+rcopy(Float64,R"as.numeric(refined_fit$opt$objective)"))<=1e-10,
 "native_baseline_health"=>all(v for (k,v) in baseline["checks"] if k!="r_gradient"))
report=Dict("scope"=>"REFERENCE_PRECISION_COMPANION_NOT_DEFAULT_PROMOTION","id"=>id,
 "fixture_sha256"=>data_hash,"native_loglik"=>baseline["native_loglik"],
 "default_loglik"=>original.logLik,"default_gradient"=>baseline["r_gradient_max"],
 "default_checks"=>baseline["checks"],"refined_loglik"=>ll,
 "refined_gradient_max"=>maximum(abs,gradient),"refined_code"=>code,
 "same_data_map_names"=>true,"source"=>source,"checks"=>checks)
mkpath(output)
open(joinpath(output,"result.toml"),"w") do io;TOML.print(io,report);end
all(values(checks)) || error("precision companion check failed; retained result")
println("BINOMIAL_PRECISION_COMPANION_PASS ",id)
