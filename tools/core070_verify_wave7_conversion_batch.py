"""Verify the wave-7 conversion batch: 7 of the 45 BLOCKED_NEEDS_JULIA_SURFACE
ledger rows in docs/dev-log/core070/required-source-case-map.json (6 distinct
comparison cases; two source_ids -- postfit/POSTFIT-SURFACE-sanity_multi and
namespace/export/sanity_multi -- share one case, same underlying Julia
surface, same precedent as wave-6's duplicate-ledger-row handling) converted
to bound, verified-from-source cases. The other 38 target rows are recorded
in the contract's `deferred` bucket with a per-row `reason` string and carry
no fabricated pass -- most are SAME-NAME, DIFFERENT-SURFACE collisions
discovered by reading the R oracle source directly (R's getREsd/
compare_Sigma_table/compare_dep_vs_two_psi/vcov.gllvmTMB_multi/
diagnostic_table/profile_targets all have a genuinely different signature or
call-shape gap from their Julia same-named counterpart; see each deferred
row's `reason`).

Three independent checks, mirroring tools/core070_verify_wave6_conversion_batch.py:
  ...
  2. verify_state(path) -- structural + hash checks against a retained run
     of tools/core070_wave7_conversion_batch.R (receipt.json, results.tsv,
...
CONTRACT_PATH = ROOT / "docs/dev-log/core070/wave7-conversion-batch-contract.json"
...
KNOWN_KINDS = {"point", "verdict", "own_consistency"}
KNOWN_FIXTURES = {"gaussian_small"}
...
def verify_contract(contract=None):
    ...
    need(c["status"] == "FROZEN_WAVE7_CONVERSION_BATCH_CONTRACT", "wrong contract status")
    ...
    for sid in all_source_ids:
        need(sid.startswith(("postfit/", "namespace/")), f"source_id {sid} missing an expected area prefix")
    ...
    for row in c["cases"]:
        ...
        if row["kind"] in ("point", "own_consistency"):
            ...
            need(row["tolerance"] <= 1e-3,
                 f"{row['case_id']}: tolerance {row['tolerance']} looser than the 1e-3 CI-endpoint tier "
                 "-- point/own_consistency cases here are paired-independent-fit tier (<=1e-4) or tighter, "
    ...
    for row in c["rejection_cases"]:
        need("expect_r_raised" in row and "expect_julia_raised" in row,
             ...
             "(per-side expectation fields for any asymmetric behavior)")
...
def check_state(contract, receipt, results_lines, julia_report):
    ...
    need(receipt["scope"] == "CORE070_WAVE7_CONVERSION_BATCH", "wrong receipt scope")
    ...
    for cid in case_ids:
        need(seen[cid] == ("PASS", "wave7_conversion"), f"{cid}: expected PASS/wave7_conversion, got {seen[cid]}")
    ...
    for row in contract["cases"]:
        ...
        if row["kind"] in ("point", "own_consistency"):
            need(jc.get("max_abs_diff") is not None or row["kind"] == "own_consistency",
                 f"{row['case_id']}: point case missing max_abs_diff")
            if row["kind"] == "point":
                need(jc["max_abs_diff"] <= row["tolerance"],
                     f"{row['case_id']}: max_abs_diff {jc.get('max_abs_diff')} exceeds tolerance {row['tolerance']}")
            else:
                need(jc.get("r_frobenius") is not None and abs(jc["r_frobenius"]) <= row["tolerance"],
                     f"{row['case_id']}: r_frobenius exceeds tolerance")
                need(jc.get("julia_frobenius") is not None and abs(jc["julia_frobenius"]) <= row["tolerance"],
                     f"{row['case_id']}: julia_frobenius exceeds tolerance")
...
def _synthetic_state(contract):
    ...
    results_lines = [f"{r['case_id']}\tPASS\twave7_conversion" for r in cases]
    ...
    for r in cases:
        ...
        elif r["kind"] == "own_consistency":
            julia_cases[r["case_id"]] = {"pass": True, "kind": r["kind"], "tolerance": r["tolerance"],
                                          "r_frobenius": 0.0, "julia_frobenius": 0.0, "error": ""}
        else:  # verdict
            julia_cases[r["case_id"]] = {"pass": True, "kind": r["kind"],
...
def run_self_test():
    ...
    def mut_flip_one_verdict(r, res, jr):
        res[0] = res[0].split("\t")[0] + "\tFAIL\twave7_conversion"
        return r, res, jr
    ...
    def mut_rejection_julia_side_flipped(r, res, jr):
        # The rejection case's expect_julia_raised is FALSE (documented
        # asymmetry) -- flipping julia_raised to True must still be rejected
        # (it no longer matches the contract's own expectation), proving the
        # verifier checks BOTH sides against their per-side expectation, not
        # just "both agree".
        cid = contract["rejection_cases"][0]["case_id"]
        jr["rejection_cases"][cid]["julia_raised"] = True
        return r, res, jr
    ...
    def mut_own_consistency_frobenius_blown(r, res, jr):
        cid = next(c["case_id"] for c in contract["cases"] if c["kind"] == "own_consistency")
        jr["cases"][cid]["julia_frobenius"] = 5.0
        jr["cases"][cid]["pass"] = True  # deliberately inconsistent
        return r, res, jr
    ...
    expect_rejected("rejection case: Julia side flipped against its documented asymmetric expectation",
    ...
    expect_rejected("own_consistency frobenius blown past tolerance while pass stays true",
                     mut_own_consistency_frobenius_blown)

    if len(rejected) < 6:
        raise AssertionError(f"only {len(rejected)} rejected mutations ran, need >=6")

    print(f"CORE070_WAVE7_CONVERSION_VERIFY_SELF_TEST_OK rejected_mutations={len(rejected)} "
...
def main():
    ...
    ap.add_argument("state", nargs="?",
                     help="retained-run directory to verify (e.g. .unlazy/core070-aghq/wave7-conversion-01)")
    ...
    contract = verify_contract()
    print(f"CORE070_WAVE7_CONVERSION_CONTRACT_OK cases={CASE_COUNT} deferred={DEFERRED_COUNT} "
    ...
    print("CORE070_WAVE7_CONVERSION_STATE_OK", args.state)
