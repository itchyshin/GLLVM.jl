# Public BFGS continuation of a healthy-gradient, false-convergence fit.
...
policy_path = joinpath(@__DIR__, "..", "docs/dev-log/core070/binomial-bfgs-policy.toml")
...
R"""
.iv <- integer(78L + 3L*length(fit_r$opt$par))
.v <- double(130L + (length(fit_r$opt$par)*(length(fit_r$opt$par)+27L))/2L)
.Call(get("C_port_ivset", asNamespace("stats")), 2, .iv, .v)
.port_initial <- .v[stats:::port_cpos[c("rel.tol","sing.tol")]]
.v[stats:::port_cpos["rel.tol"]] <- 1e-14
.port_reltol_only <- .v[stats:::port_cpos[c("rel.tol","sing.tol")]]
stopifnot(identical(as.numeric(.port_initial),c(1e-10,1e-10)),
          identical(as.numeric(.port_reltol_only),c(1e-14,1e-10)))
original_fit <- fit_r
...
for reltol in policy["relative_tolerances"]
    ...
    .public_controls <- list(rel.tol=reltol,eval.max=2000,iter.max=1500)
    if (identical(reltol,1e-14)) .public_controls$sing.tol <- 1e-14
    candidate_fit <- gllvmTMB(
      ...
      control=gllvmTMBcontrol(n_init=1L,se=FALSE,start_from=parent_fit,
       optArgs=list(control=.public_controls)))
...
if !attempts[end]["healthy"] && attempts[end]["code"] != 0 &&
        all(isfinite,attempts[end]["gradient"]) && maximum(abs,attempts[end]["gradient"])<=1e-4
    R"""
    parent_fit <- candidate_fit
    candidate_fit <- gllvmTMB(
      value ~ 0 + trait + latent(0 + trait | site,d=K,unique=FALSE),
      data=df_long,unit="site",trait="trait",family=fam_obj,weights=weights_vec,
      control=gllvmTMBcontrol(n_init=1L,se=FALSE,start_from=parent_fit,
        optimizer="optim",optArgs=list(method="BFGS",control=list(reltol=1e-12,maxit=2000))))
    stopifnot(identical(original_fit$tmb_data,candidate_fit$tmb_data),
      identical(original_fit$tmb_obj$env$map,candidate_fit$tmb_obj$env$map),
      identical(names(original_fit$opt$par),names(candidate_fit$opt$par)))
    .snapshot_value <- .snapshot(candidate_fit)
    """
    push!(attempts,snapshot("public_BFGS_start_from","1.0e-12"))
end
...
report=Dict("scope"=>policy["scope"],"id"=>id,"fixture_sha256"=>data_hash,
 ...
 "port_initial_relative_singular"=>rcopy(Vector{Float64},R".port_initial"),
 "port_relative_only_relative_singular"=>rcopy(Vector{Float64},R".port_reltol_only"),
 "final_singular_tolerance"=>policy["final_singular_tolerance"],
...
println("BINOMIAL_BFGS_CONTINUATION_PASS ",id)
