#!/usr/bin/env bash
# T4 Gaussian pre-run — Totoro dry-run launcher (M2 day-2, M2-S3 prep).
#
# Authority: docs/dev-log/core070/t4-totoro-estimate-2026-09-05.md
# Cell 1 of 3: gaussian p=20 n=500 K=2 seed=42 (each-own-optimum, not gated).
#
# NEEDS SHINICHI COMPUTE-GO — estimate-only until maintainer approves launch.
# Default: print rsync + ssh commands and exit 0.
# Launch only when GLLVM_TOTORO_LAUNCH=1 is set explicitly in the shell.
#
# Usage:
#   tools/t4_totoro_gaussian_prerun.sh              # dry-run (default)
#   GLLVM_TOTORO_LAUNCH=1 tools/t4_totoro_gaussian_prerun.sh
...
echo "=== T4 Gaussian pre-run — Totoro queue spec (DO NOT SUBMIT without compute-go) ==="
echo "Authority: docs/dev-log/core070/t4-totoro-estimate-2026-09-05.md"
...
echo "REMOTE"
echo ""
echo "NEEDS SHINICHI COMPUTE-GO — set GLLVM_TOTORO_LAUNCH=1 to execute the above."
echo ""
