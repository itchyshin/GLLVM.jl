# Public fixed-to-free nlminb diagnostic; original fixture and gates unchanged.
...
R"""
 ...
 final_fit <- gllvmTMB(
   ...
   control=gllvmTMBcontrol(n_init=1L,se=FALSE,start_from=warm_fit,
      optArgs=list(control=list(rel.tol=1e-12,sing.tol=1e-12,eval.max=2000,iter.max=1500))))
...
report=Dict("scope"=>"ORIGINAL_STUDENT_PUBLIC_FIXED_TO_FREE_NLMINB",
...
println("ORIGINAL_STUDENT_PUBLIC_NLMINB_QUALIFIED")
