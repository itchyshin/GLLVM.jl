...
using GLLVModels, TOML, SHA, Test
...
@testset "Julia AGHQ input dialect" begin
    for (name,value) in [("whole_float_rejected",9.0),("auto_string_rejected","auto")]
        rejected=try GLLVModels._aghq_request(value);false catch err;err isa ArgumentError end
        @test rejected
...
open(ENV["CORE070_CONTROL_OUTPUT"],"w") do io
    TOML.print(io,Dict("schema"=>1,"scope"=>spec["scope"],"cases"=>results,
        ...
        "package_root"=>realpath(dirname(dirname(pathof(GLLVModels)))),"dialect_checks"=>dialect))
end
