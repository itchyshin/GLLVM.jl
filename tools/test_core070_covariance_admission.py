...
MANIFEST=ROOT/'docs/dev-log/core070/covariance-admission-subset.json'
SOURCE=ROOT/'.unlazy/core070-aghq/oracle-source/readback'


class SourceAdmission(unittest.TestCase):
    def setUp(self):
        self.assertTrue((ROOT/'tools/core070_covariance_admission.py').is_file(), 'frozen-source replay runner is missing')
        self.runner=importlib.import_module('core070_covariance_admission')
    ...
    def test_omitted_case_and_stale_source_launch_nothing(self):
        for label in ('omitted','stale'):
            ...
            else:d['source_pins']['R/brms-sugar.R']='0'*64
