# Non-Gaussian missing predictor (mi() Phase 5a) — Poisson-log smallest slice.
#
# A Poisson-log GLLVM with ONE missing site-level continuous predictor x_s and a
# broadcast slope b_x, x_s ~ N(μ_x, σ_x²), integrated by full-information ML:
#   y[t,s] ~ Poisson(exp(η[t,s])),  η[t,s] = β_t + b_x x_s + (Λ z_s)_t,  z_s ~ N(0,I_K)
#
# Per site:
#   • x_s OBSERVED  — fold b_x x_s into the offset, Laplace over z only (reuse the
#       existing primal mode + the implicit differentiable Newton step), plus the
#       full x-prior density logN(x_s; μ_x, σ_x²).
#   • x_s MISSING   — integrate x_s: augment the latent to (z, x), joint Laplace
#       with the rank-1 bordered Fisher Hessian
#         H = [ Λ'WΛ+I        b_x Λ'W·1 ;
#               b_x 1'WΛ   b_x²Σ_t W_t + 1/σ_x² ],   W_t = μ_t (Poisson),
#       marginal  ℓ(ẑ,x̂) − ½ẑ'ẑ − ½(x̂−μ_x)²/σ_x² − ½log σ_x² − ½logdet(H).
#       (The (2π) constants cancel as in the K-dim code; this limits to the
#       observed-at-μ_x value as σ_x→0. Verified vs 2-D Gauss–Hermite quadrature.)
#
# AD-clean via the implicit "one differentiable Newton step at the mode" trick
# (mirrors laplace_grad.jl) on the (K+1) augmented system. Smallest slice =
# Poisson-log marginal primitive + oracle tests (no exported fitter; other
# families + the hand-coded (K+1) adjoint are follow-ons). See
# docs/dev-log/2026-06-13-nongaussian-mi-design.md.

using LinearAlgebra

# Primal augmented (z, x) Newton mode (Float64; called on primal parameter values).
function _poisson_mode_xs(y, Λ, β, b_x::Real, μ_x::Real, σ_x2::Real;
                          maxiter::Integer = 100, tol::Real = 1e-9)
    p, K = size(Λ)
    z = zeros(K)
    x = float(μ_x)
    for _ in 1:maxiter
        η = _clamp_eta.(β .+ b_x * x .+ Λ * z)
        μ = exp.(η)
        s = y .- μ
        gz = Λ' * s .- z
        gx = b_x * sum(s) - (x - μ_x) / σ_x2
        A = Λ' * (μ .* Λ) + I
        c = b_x .* (Λ' * μ)
        dd = b_x^2 * sum(μ) + 1 / σ_x2
        H = [A c; c' dd]
        Δ = H \ vcat(gz, gx)
        all(isfinite, Δ) || break
        z = z .+ Δ[1:K]
        x = x + Δ[K + 1]
        maximum(abs, Δ) < tol && break
    end
    return z, x
end

# Differentiable per-site Poisson marginal with the site-predictor x_s (observed
# or missing). β, Λ, b_x, μ_x, σ_x2 may carry duals; the mode is primal.
function _poisson_site_xs_diffable(y, Λ, β, x_obs, b_x, μ_x, σ_x2)
    p, K = size(Λ)
    Λv = ForwardDiff.value.(Λ)
    βv = ForwardDiff.value.(β)
    bxv = ForwardDiff.value(b_x)
    mxv = ForwardDiff.value(μ_x)
    sx2v = ForwardDiff.value(σ_x2)

    if x_obs !== nothing
        # observed x: offset b_x x_obs, mode over z only.
        ẑ = _laplace_mode(Poisson(), y, ones(Int, p), Λv, βv, LogLink();
                          offset = bxv * x_obs)
        off = b_x * x_obs
        η = _clamp_eta.(β .+ off .+ Λ * ẑ)
        μ = exp.(η)
        A = Λ' * (μ .* Λ) + I
        z = ẑ .+ (A \ (Λ' * (y .- μ) .- ẑ))
        ηz = _clamp_eta.(β .+ off .+ Λ * z)
        μz = exp.(ηz)
        Az = Λ' * (μz .* Λ) + I
        ℓ = zero(eltype(z))
        @inbounds for t in 1:p
            ℓ += _pois_logpmf(μz[t], y[t])
        end
        lpx = -0.5 * log(2π) - 0.5 * log(σ_x2) - 0.5 * (x_obs - μ_x)^2 / σ_x2
        return ℓ - 0.5 * dot(z, z) - 0.5 * logdet(Az) + lpx
    else
        # missing x: augmented (z, x) joint Laplace.
        ẑ, x̂ = _poisson_mode_xs(y, Λv, βv, bxv, mxv, sx2v)
        η = _clamp_eta.(β .+ b_x * x̂ .+ Λ * ẑ)
        μ = exp.(η)
        s = y .- μ
        gz = Λ' * s .- ẑ
        gx = b_x * sum(s) - (x̂ - μ_x) / σ_x2
        A = Λ' * (μ .* Λ) + I
        c = b_x .* (Λ' * μ)
        dd = b_x^2 * sum(μ) + 1 / σ_x2
        H = [A c; c' dd]
        Δ = H \ vcat(gz, gx)
        z = ẑ .+ Δ[1:K]
        x = x̂ + Δ[K + 1]
        ηz = _clamp_eta.(β .+ b_x * x .+ Λ * z)
        μz = exp.(ηz)
        Az = Λ' * (μz .* Λ) + I
        cz = b_x .* (Λ' * μz)
        ddz = b_x^2 * sum(μz) + 1 / σ_x2
        Hz = [Az cz; cz' ddz]
        ℓ = zero(eltype(z))
        @inbounds for t in 1:p
            ℓ += _pois_logpmf(μz[t], y[t])
        end
        return ℓ - 0.5 * dot(z, z) - 0.5 * (x - μ_x)^2 / σ_x2 -
               0.5 * log(σ_x2) - 0.5 * logdet(Hz)
    end
end

"""
    laplace_loglik_site_xs(family, y, n, Λ, β, link; x_obs, b_x, μ_x, σ_x2, ...) -> Float64

Per-site Laplace marginal for a non-Gaussian GLLVM with one site-level predictor
`x_s` (observed value `x_obs`, or `nothing` if missing-and-integrated), broadcast
slope `b_x`, predictor model `x_s ~ N(μ_x, σ_x²)`. Smallest slice: `Poisson()` +
`LogLink` only.
"""
function laplace_loglik_site_xs(family, y::AbstractVector, n::AbstractVector,
                                Λ::AbstractMatrix, β::AbstractVector, link;
                                x_obs, b_x, μ_x, σ_x2)
    (family isa Poisson && link isa LogLink) ||
        throw(ArgumentError("laplace_loglik_site_xs: smallest slice supports Poisson() + LogLink only."))
    return _poisson_site_xs_diffable(y, Λ, β, x_obs, b_x, μ_x, σ_x2)
end

"""
    marginal_loglik_laplace_xs(family, Y, N, Λ, β, link; x, b_x, μ_x, σ_x2, ...) -> Float64

Total Laplace log-marginal over sites for a Poisson-log GLLVM with one missing
site-level continuous predictor integrated by FIML (mi() Phase 5a). `x` is a
length-n vector (entries may be `missing`/`NaN`). Observed sites contribute the
joint `(y_s, x_s)` density; missing sites contribute the marginal of `y_s` with
`x_s` integrated out.
"""
function marginal_loglik_laplace_xs(family, Y::AbstractMatrix, N::AbstractMatrix,
                                    Λ::AbstractMatrix, β::AbstractVector, link;
                                    x::AbstractVector, b_x, μ_x, σ_x2)
    acc = zero(promote_type(eltype(Λ), eltype(β), typeof(b_x), typeof(μ_x), typeof(σ_x2)))
    @inbounds for s in axes(Y, 2)
        xs = x[s]
        x_obs = (ismissing(xs) || (xs isa Real && isnan(xs))) ? nothing : Float64(xs)
        acc += laplace_loglik_site_xs(family, view(Y, :, s), view(N, :, s), Λ, β, link;
                                      x_obs = x_obs, b_x = b_x, μ_x = μ_x, σ_x2 = σ_x2)
    end
    return acc
end
