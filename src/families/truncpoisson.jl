...
"""
function fit_truncpoisson_gllvm(Y::AbstractMatrix{<:Integer}; K::Integer,
        link::Link = LogLink(),
        β_init = nothing, Λ_init = nothing,
        g_tol::Real = 1e-5, iterations::Integer = 500,
        newton_maxiter::Integer = 100, newton_tol::Real = 1e-9)
    p, n = size(Y)
    minimum(Y) ≥ 1 || throw(ArgumentError(
        "fit_truncpoisson_gllvm: zero-truncated Poisson requires all Y ≥ 1; got min(Y) = $(minimum(Y))"))
    rr = rr_theta_len(p, K)
