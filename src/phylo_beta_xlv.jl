# Internal S1 proof for the phylo x Beta x predictor-informed LV target.
#
# Model:
#   epsilon_s ~ N(0, I_K)
...
function _phylo_beta_xlv_validate(Y::AbstractMatrix, beta::AbstractVector,
        Lambda::AbstractMatrix, alpha_lv::AbstractMatrix, phi::Real,
        phy::AugmentedPhy, X_lv::AbstractMatrix, link::Link)
    link isa LogitLink ||
        throw(ArgumentError("phylo Beta X_lv S1 is currently logit-link only"))
    isfinite(float(phi)) && phi > 0 ||
        throw(ArgumentError("Beta precision phi must be positive and finite"))
    p, n = size(Y)
    p == phy.n_leaves ||
    ...
    all(0 .< Y .< 1) || throw(ArgumentError("Y must lie strictly between 0 and 1 for Beta"))
    length(beta) == p ||
...
function _phylo_beta_xlv_logpost(Y, beta, Lambda, alpha_lv, phi,
    ...
    @inbounds for s in 1:n, t in 1:p
        ...
        mu_ts = _clamp_mu(fam, linkinv(LogitLink(), _clamp_eta(eta_ts)))
        q += _glm_logpdf(fam, mu_ts, 1, Y[t, s])
    end
    return q
end

function _phylo_beta_xlv_grad_hessian(Y, beta, Lambda, alpha_lv, phi,
    ...
    @inbounds for s in 1:n, t in 1:p
        ...
        mu_ts = _clamp_mu(fam, linkinv(LogitLink(), eta_c))
        ...
        weight_ts = hessian === :fisher ?
            ...
            _glm_obs_weight(fam, mu_ts, 1, me_ts, Y[t, s], LogitLink(), eta_c)
        uidx = n_z + leaf_pos[t]
...
function _phylo_beta_xlv_mode(Y, beta, Lambda, alpha_lv, phi,
    ...
    for _ in 1:maxiter
        grad, H = _phylo_beta_xlv_grad_hessian(
            Y, beta, Lambda, alpha_lv, phi, mean_eta, Q, leaf_pos, eps, u)
        cholH = try
        ...
        q0 = _phylo_beta_xlv_logpost(Y, beta, Lambda, alpha_lv, phi,
                                      mean_eta, Q, leaf_pos, eps, u)
        accepted = false
        step = 1.0
        n_z = n * K
        ...
        for _half in 1:30
            ...
            q1 = _phylo_beta_xlv_logpost(Y, beta, Lambda, alpha_lv, phi,
    ...
    _, H = _phylo_beta_xlv_grad_hessian(Y, beta, Lambda, alpha_lv, phi,
                                         mean_eta, Q, leaf_pos, eps, u;
        hessian = _default_hessian(Beta(phi, 1.0), LogitLink()))
    cholH = try
        cholesky(Symmetric(H))
    catch
        return nothing, nothing, nothing
...
"""
    _phylo_beta_xlv_marginal_loglik(Y, beta, Lambda, alpha_lv, phi, sigma2_phy, phy, X_lv; kwargs...)

Internal Laplace marginal log-likelihood for the phylo x Beta x
predictor-informed latent-score S1 proof. The only admitted link is `LogitLink`.
The integrated variables are the site-score innovations and the augmented-tree
phylogenetic random intercept. The shared Beta precision is an outer nuisance
parameter in the point wrapper, not part of the interval target.
"""
function _phylo_beta_xlv_marginal_loglik(Y::AbstractMatrix,
        beta::AbstractVector, Lambda::AbstractMatrix, alpha_lv::AbstractMatrix,
        phi::Real, sigma2_phy::Real, phy::AugmentedPhy, X_lv::AbstractMatrix;
        link::Link = LogitLink(), maxiter::Integer = 80, tol::Real = 1e-9)
    _phylo_beta_xlv_validate(Y, beta, Lambda, alpha_lv, phi, phy, X_lv, link)
    ...
    eps_hat, u_hat, cholH = _phylo_beta_xlv_mode(
        Yc, beta_c, Lambda_c, alpha_c, float(phi), mean_eta, Q, leaf_pos;
        maxiter = maxiter, tol = tol)
    eps_hat === nothing && return -Inf
    q = _phylo_beta_xlv_logpost(Yc, beta_c, Lambda_c, alpha_c, float(phi),
                                 mean_eta, Q, leaf_pos, eps_hat, u_hat)
    return q + 0.5 * logdet(cholQ) - 0.5 * logdet(cholH)
end

function _phylo_beta_xlv_unpack_packed(theta::AbstractVector, p::Integer,
    ...
    length(theta) == n_expected || throw(ArgumentError(
        ...
        "(p=$p + alpha_lv=$(q_lv * K) + rr=$rr + log_phi=1 + log_sigma2=1)"))
    ...
    cursor += rr
    phi = exp(theta[cursor + 1])
    sigma2_phy = exp(theta[cursor + 2])
    return beta, Lambda, alpha_lv, phi, sigma2_phy
end

function _phylo_beta_xlv_effects_from_packed(theta::AbstractVector,
        p::Integer, K::Integer, q_lv::Integer)
    _, Lambda, alpha_lv, _, _ = _phylo_beta_xlv_unpack_packed(theta, p, K, q_lv)
    return vec(Matrix(Lambda * transpose(alpha_lv)))
end

function _phylo_beta_xlv_nll_packed(theta::AbstractVector, Y::AbstractMatrix,
        p::Integer, K::Integer, phy::AugmentedPhy, X_lv::AbstractMatrix;
        q_lv::Integer, maxiter::Integer = 80, tol::Real = 1e-9)
    beta, Lambda, alpha_lv, phi, sigma2_phy =
        _phylo_beta_xlv_unpack_packed(theta, p, K, q_lv)
    ll = _phylo_beta_xlv_marginal_loglik(Y, beta, Lambda, alpha_lv,
                                          phi, sigma2_phy, phy, X_lv;
                                          maxiter = maxiter, tol = tol)
    return isfinite(ll) ? -ll : 1e12
end

function _phylo_beta_xlv_theta0(Y::AbstractMatrix, K::Integer,
        X_lv::AbstractMatrix; beta_init = nothing, Lambda_init = nothing,
        alpha_lv_init = nothing, phi_init::Real = 2.0,
        sigma2_phy_init::Real = 0.5)
    p, n = size(Y)
    q_lv = size(X_lv, 2)
    K > 0 || throw(ArgumentError("K must be positive"))
    q_lv > 0 || throw(ArgumentError("X_lv must have at least one column"))
    phi_init > 0 && isfinite(float(phi_init)) ||
        throw(ArgumentError("phi_init must be positive and finite"))
    sigma2_phy_init > 0 ||
        throw(ArgumentError("sigma2_phy_init must be positive"))

    beta0 = beta_init === nothing ?
        [linkfun(LogitLink(), clamp(sum(Y[t, s] for s in 1:n) / n, 1e-6, 1 - 1e-6))
         for t in 1:p] :
    ...
    return vcat(beta0, vec(alpha0), pack_lambda(Lambda0), log(float(phi_init)),
                log(float(sigma2_phy_init)))
end

"""
    _fit_phylo_beta_xlv(Y, phy; K, X_lv, kwargs...) -> NamedTuple

Private point-route wrapper for the phylo x Beta x predictor-informed LV S1
...
"""
function _fit_phylo_beta_xlv(Y::AbstractMatrix, phy::AugmentedPhy;
        K::Integer, X_lv::AbstractMatrix,
        beta_init = nothing, Lambda_init = nothing, alpha_lv_init = nothing,
        phi_init::Real = 2.0, sigma2_phy_init::Real = 0.5,
    ...
    size(X_lv, 1) == n ||
        throw(ArgumentError("X_lv rows ($(size(X_lv, 1))) must equal size(Y,2)=$n"))
    q_lv = size(X_lv, 2)
    theta0 = _phylo_beta_xlv_theta0(Y, K, X_lv;
                                     beta_init = beta_init,
    ...
    nll = theta -> begin
        v = try
            _phylo_beta_xlv_nll_packed(theta, Y, p, K, phy, X_lv;
                                        q_lv = q_lv,
    ...
    beta_hat, Lambda_hat, alpha_hat, phi_hat, sigma2_hat =
        _phylo_beta_xlv_unpack_packed(theta_hat, p, K, q_lv)
    return (beta = collect(beta_hat), Lambda = Matrix(Lambda_hat),
            alpha_lv = Matrix(alpha_hat), phi = phi_hat,
...
function _phylo_beta_xlv_profile_indices(indices, nb::Integer)
...
function _phylo_beta_xlv_constrained_refit(nll::Function,
    ...
    for weight in penalty_weights
        obj = function (theta)
            ...
            b = _phylo_beta_xlv_effects_from_packed(theta, p, K, q_lv)[entry]
    ...
    b_c = try
        _phylo_beta_xlv_effects_from_packed(theta_c, p, K, q_lv)[entry]
    catch
        NaN
    end
...
function _phylo_beta_xlv_profile_eta_realized(fit, Y::AbstractMatrix,
    ...
    length(phi_bounds) == 2 &&
        0 < phi_bounds[1] < phi_bounds[2] ||
        throw(ArgumentError("phi_bounds must be a positive increasing pair"))
    p, K = size(fit.Lambda)
    q_lv = size(X_lv, 2)
    nb = p * q_lv
    idx = _phylo_beta_xlv_profile_indices(entries, nb)
    ...
    nll = theta -> _phylo_beta_xlv_nll_packed(theta, Y, p, K, phy, X_lv;
                                               q_lv = q_lv,
                                               maxiter = newton_maxiter,
                                               tol = newton_tol)
    nll0 = nll(theta0)
    cutoff = quantile(Chisq(1), level)
    b_hat = _phylo_beta_xlv_effects_from_packed(theta0, p, K, q_lv)
    _, _, _, phi_hat, _ = _phylo_beta_xlv_unpack_packed(theta0, p, K, q_lv)
    ...
    for entry in idx
        ...
        refit_truth = _phylo_beta_xlv_constrained_refit(
        ...
        if profile_endpoints
            ...
            function dev_at(c, start)
                refit = _phylo_beta_xlv_constrained_refit(
    ...
    phi_ok = phi_bounds[1] < phi_hat < phi_bounds[2]
    return (term = terms, estimate = estimates, target = targets,
            ...
            phi = phi_hat, phi_bounds = phi_bounds,
            phi_ok = phi_ok,
            level = level, method = :profile_eta_realized,
            pd_hessian = phi_ok && all(converged) &&
